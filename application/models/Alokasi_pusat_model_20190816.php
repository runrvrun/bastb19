	...
	class Alokasi_pusat_model extends CI_Model
	{
		...
		function get($id = null, $id_kontrak_pusat = null,  $start = null, $length = null, $col = null, $dir = null, $filter = null, $search = null)
		{
			...
			$qry =	"	
				...
				hd.`merk`, barang.`jenis_barang`, dt.id, dt.`id_kontrak_pusat`, dt.`id_provinsi`, p.`nama_provinsi`, dt.`id_kabupaten`, 
		...
		{
			if(empty($col)) $col = 'alo.created_at';
			if(empty($dir)) $dir = 'DESC';
			$qry =	"SELECT * FROM (
				SELECT alo.id, alo.id_kontrak_pusat,
					hd.`tahun_anggaran`,
					hd.`no_kontrak`,
					CONCAT(DATE_FORMAT(hd.`periode_mulai`,'%d-%m-%Y'),' s/d ',DATE_FORMAT(hd.`periode_selesai`,'%d-%m-%Y')) periode,
					hd.`nama_barang`,
					...
					sum(tb_baphp_reguler.jumlah_barang) as jumlah_baphp, sum(tb_bastb_pusat.jumlah_barang) as jumlah_bastb		
				FROM 
				...
				LEFT JOIN tb_baphp_reguler ON tb_baphp_reguler.id_alokasi_pusat is not null AND tb_baphp_reguler.id_alokasi_pusat = alo.id 
				LEFT JOIN tb_bastb_pusat ON tb_bastb_pusat.id_alokasi_pusat is not null AND tb_bastb_pusat.id_alokasi_pusat = alo.id
				WHERE 
				CASE
				...
				and pen.id = ".$this->session->userdata('logged_in')->id_penyedia_pusat : "");
				if(isset($filter)) {
					$filter = str_replace('jumlah_barang','alo.jumlah_barang',$filter);
					$qry .= $filter;
				}
				$qry .= " GROUP BY 
				hd.`tahun_anggaran`, hd.`no_kontrak`, hd.`periode_mulai`, hd.`periode_selesai`, hd.`nama_barang`, 
				...
				$qry .=	" UNION 	
				SELECT  dt.id, alo.id_kontrak_pusat,
					hd.`tahun_anggaran`,
					hd.`no_kontrak`,
					CONCAT(DATE_FORMAT(hd.`periode_mulai`,'%d-%m-%Y'),' s/d ',DATE_FORMAT(hd.`periode_selesai`,'%d-%m-%Y')) periode,
					hd.`nama_barang`,
					...
					sum(tb_baphp_persediaan.jumlah_barang) as jumlah_baphp, sum(tb_bastb_persediaan.jumlah_barang) as jumlah_bastb,
					hd.created_at as kontrak_created, dt.created_at as alokasi_created
				FROM 
				...
				INNER JOIN tb_provinsi p ON p.id = dt.`id_provinsi`
				INNER JOIN tb_kabupaten k ON k.id = dt.`id_kabupaten`
				LEFT JOIN tb_baphp_persediaan ON tb_baphp_persediaan.id_alokasi_persediaan_pusat IS NOT NULL AND tb_baphp_persediaan.id_alokasi_persediaan_pusat = dt.id
				LEFT JOIN tb_bastb_persediaan ON tb_bastb_persediaan.id_alokasi_persediaan_pusat IS NOT NULL AND tb_bastb_persediaan.id_alokasi_persediaan_pusat = dt.id
				WHERE 
				...
				) alokasi_union ORDER BY kontrak_created DESC, id_kontrak_pusat DESC, id DESC";
		...
		function total_unit($id = null, $id_kontrak_pusat = null)
		{
			$qry =	"	
