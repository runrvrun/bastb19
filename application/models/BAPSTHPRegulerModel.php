<?php
	class BAPSTHPRegulerModel extends CI_Model
	{
		...
		function GetAllAjaxCount()
		{
			$qry =	"	
				SELECT 
					...
					peny.nama_penyedia_pusat as pihak_penyerah, 
					bap.nama_penyerah,
				...
				LEFT JOIN tb_penyedia_pusat peny ON bap.id_penyerah = peny.id
				...
				".(isset($this->session->userdata('logged_in')->id_penyedia_pusat) ? " and id_penyerah = ".$this->session->userdata('logged_in')->id_penyedia_pusat : "")."
					";
		...
		function GetSearchAjaxCount($search, $filter = '')
		{
			$qry =	"
				...
				AND (
					...
					peny.nama_penyedia_pusat LIKE '%".$search."%' or
		...
		{
			$this->db->insert('tb_bapsthp_reguler',$data);
		}

		function Update($id, $data)
		{
			$this->db->where('id', $id);
			$this->db->update('tb_bapsthp_reguler', $data);
		}

		function Delete($id)
		{
			$this->db->delete('tb_bapsthp_reguler', array('id' => $id));
		}
