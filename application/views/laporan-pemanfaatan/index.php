...
<div class="col-md-12 npl npr mb20 dt-buttons">
  ...
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <!-- <a href="<?php echo base_url('Bastb_pusat/index?id_alokasi='.$bastb_pusat->id_alokasi_pusat); ?>"> -->
    ...
    <?php } ?>
    <?php if($bolehtambah && isset($bastb_pusat)) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Laporan_pemanfaatan/create?id_bastb_pusat=').$bastb_pusat->id; ?>">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Laporan_pemanfaatan/upload_file"); ?>',
    ...
    success: function (file, response) {      
      window.location="<?php echo base_url('Laporan_pemanfaatan/index?id_bastb_pusat='.$bastb_pusat->id); ?>";      
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Laporan_pemanfaatan/index_json"); ?>',
        "data": {
           id_bastb_pusat : '<?php echo $bastb_pusat->id; ?>',
        },
        "dataType": "json",
    },
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Laporan_pemanfaatan/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/laporan_pemanfaatan/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Laporan_pemanfaatan/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/laporan_pemanfaatan/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/laporan_pemanfaatan/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/laporan_pemanfaatan/"+obj[i], "#embedPDF_"+i);
