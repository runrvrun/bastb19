...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-4" style="height: 120px; border: solid 1px; ">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp_persediaan_norangka/create?id_baphp_persediaan='.$baphp_persediaan->id); ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Pilih No. Rangka & No. Mesin</span>
    </div>
    <?php } ?>    
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
  ...
  <div style="float: right; margin-right: -30px;">
    <div class="col-sm-6" style="width: 200px; height: 100px; text-align: center; padding-top: 15px;">
      ...
      <b style="font-size: 22px; "><span class="Count"><?php echo count($baphp_persediaan_norangka); ?></span></b><br> Unit
    </div>
  </div>
</div>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    success: function (file, response) {      
      <?php if(isset($baphp_persediaan)){ ?>
      window.location="<?php echo base_url('Norangka/index?id='.$baphp_persediaan->id); ?>";
      <?php }else{ ?>
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Baphp_persediaan_norangka/index_json"); ?>',
        <?php if(isset($baphp_persediaan)){ ?>
        "data": {
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Baphp_persediaan_norangka/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id_norangka},
  ...
  function removeImage(id_norangka, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp_persediaan_norangka/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_norangka : id_norangka, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($baphp_persediaan)){ ?>
          window.location="<?php echo base_url('Baphp_persediaan_norangka/index?id='.$baphp_persediaan->id); ?>";
          <?php }else{ ?>
            window.location="<?php echo base_url('Baphp_persediaan_norangka'); ?>";
          <?php } ?>
        },
        error: function(jqXHR, textStatus, errorThrown) {             
          <?php if(isset($baphp_persediaan)){ ?>
