...
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  <h4><?php echo $bastb_provinsi->nama_penyedia_provinsi; ?></h4>
...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-4" style="height: 120px; border: solid 1px; ">
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Bastb_provinsi/index?id_alokasi_provinsi='.$bastb_provinsi->id_alokasi_provinsi); ?>">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Bastb_provinsi_norangka/create?id_bastb_provinsi='.$bastb_provinsi->id); ?>">
  ...
  <div style="float: right; margin-right: -30px;">
    <div class="col-sm-6" style="width: 200px; height: 100px; text-align: center; padding-top: 15px;">
      ...
      <b style="font-size: 22px; "><span class="Count"><?php echo count($bastb_provinsi_norangka); ?></span></b><br> Unit
    </div>
  </div>
</div>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    success: function (file, response) {      
      <?php if(isset($bastb_provinsi)){ ?>
      window.location="<?php echo base_url('Norangka/index?id='.$bastb_provinsi->id); ?>";
      <?php }else{ ?>
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Bastb_provinsi_norangka/index_json"); ?>',
        <?php if(isset($bastb_provinsi)){ ?>
        "data": {
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Bastb_provinsi_norangka/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id_norangka},
  ...
  function removeImage(id_norangka, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Bastb_provinsi_norangka/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_norangka : id_norangka, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($bastb_provinsi)){ ?>
          window.location="<?php echo base_url('Bastb_provinsi_norangka/index?id='.$bastb_provinsi->id); ?>";
          <?php }else{ ?>
            window.location="<?php echo base_url('Bastb_provinsi_norangka'); ?>";
          <?php } ?>
        },
        error: function(jqXHR, textStatus, errorThrown) {             
          <?php if(isset($bastb_provinsi)){ ?>
