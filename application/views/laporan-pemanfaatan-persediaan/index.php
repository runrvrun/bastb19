...
<div class="col-md-12 npl npr mb20 dt-buttons">
  ...
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Bastb_persediaan/index?id_alokasi_persediaan='.$bastb_persediaan->id_alokasi_persediaan_pusat); ?>">
        <img src="<?php echo base_url('assets/ico/back_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Kembali</span>
    </div>
    <?php } ?>
    <?php if($bolehtambah && isset($bastb_persediaan)) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Laporan_pemanfaatan_persediaan/create?id_bastb_persediaan=').$bastb_persediaan->id; ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input Laporan Pemanfaatan</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
  ...
  <thead>
    ...
    <?php foreach($cols as $key=>$val){ ?>
      <?php
        ...
        if($val['column'] == 'periode_mulai'){
          $val['caption'] = 'Periode Penggunaan';
        }
      ?>
...
<div class="modal fade" id="preview-data" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="color:black; width: 95%;">
        <div class="modal-content" style="height: auto;">
            ...
            <div class="modal-header">
                Preview Data Laporan Pemanfaatan
            </div>
            <div class="modal-body">
              <div class="row">
                <div class="col-sm-4">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Laporan_pemanfaatan_persediaan/upload_file"); ?>',
    ...
    success: function (file, response) {      
        window.location="<?php echo base_url('Laporan_pemanfaatan_persediaan/index?id_bastb_persediaan='.$bastb_persediaan->id); ?>";
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Laporan_pemanfaatan_persediaan/index_json"); ?>',
        <?php if(isset($bastb_persediaan)){ ?>
        "data": {
           id_bastb_persediaan : '<?php echo $bastb_persediaan->id; ?>',
        },
      <?php }elseif(isset($bastb_persediaan)){ ?>
        "data": {
           id_bastb_persediaan : '<?php echo $bastb_persediaan->id; ?>',
        },
        <?php } ?>
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Laporan_pemanfaatan_persediaan/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/laporan_pemanfaatan_persediaan/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Laporan_pemanfaatan_persediaan/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        ...
        success: function(data) { 
          ...
          window.location="<?php echo base_url('Laporan_pemanfaatan_persediaan/index?id_bastb_persediaan='.$bastb_persediaan->id); ?>";
          <?php }else{ ?>
            window.location="<?php echo base_url('Laporan_pemanfaatan_persediaan'); ?>";
          <?php } ?>
        },
        error: function(jqXHR, textStatus, errorThrown) {             
          <?php if(isset($bastb_persediaan)){ ?>
      ...
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/laporan_pemanfaatan_persediaan/"+obj[i], "#embedPDF_"+i);
