...
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  <h4><?php echo $kontrak_pusat[0]->nama_penyedia_pusat; ?></h4>
  <h2><?php echo $kontrak_pusat[0]->no_kontrak; ?></h2>
  <h4><?php echo date('d M Y', strtotime($kontrak_pusat[0]->periode_mulai))." - ".date('d M Y', strtotime($kontrak_pusat[0]->periode_selesai)); ?> 
  (tersisa <?php echo max(round((strtotime($kontrak_pusat[0]->periode_selesai) - time()) / (60 * 60 * 24)),0) ?> hari)</h4>
  <div class="btn-group" role="group" aria-label="Basic example">
    <button type="button" class="btn btn-default btn-lg" style="width: 250px; height: 50px;white-space: normal;">
      <!-- <i class="glyphicon glyphicon-cog"></i><br> -->
      <b><?php echo $kontrak_pusat[0]->nama_barang ?></b><br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 250px; height: 50px;white-space: normal;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo $kontrak_pusat[0]->merk; ?></b><br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 200px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo number_format($kontrak_pusat[0]->tanggal, 0); ?></b> UNIT<br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 300px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      Rp <b><?php echo number_format($kontrak_pusat[0]->judul, 0); ?></b><br>
    </button>
  </div>
</div>
<hr>
...
<div class="col-md-12 npl npr mb20 dt-buttons">
  ...
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Informasi_terkait/create'); ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input Pengawasan</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
...
<div class="modal fade" id="upload-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="color:black;">
        <div class="modal-content">
            <div class="modal-header">
                Upload File Untuk Laporan Pengawasan <b><i class="title"></i></b>
            </div>
            <div class="modal-body">
              <div class="col_full">
...
<div class="modal fade" id="preview-data" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="color:black; width: 95%;">
        <div class="modal-content" style="height: auto;">
            ...
            <div class="modal-header">
                Preview Laporan Pengawasan Kontrak
            </div>
            <div class="modal-body">
              <div class="row">
                <div class="col-sm-4">
  ...
  <div class="modal-dialog" style="color:black; width: 1000px;">
      <div class="modal-content">
          ...
          <div class="modal-body" style="width: 1000px; ">
            <div class="row">
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="1" id="chkTanggal" checked>
                    <label class="form-check-label" for="chkTanggal">Tanggal</label>
                </div>
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="2" id="chkShowProvinsi" checked>
                    <label class="form-check-label" for="chkShowProvinsi">Provinsi</label>
                </div>
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="3" id="chkShowKabupaten" checked>
                    <label class="form-check-label" for="chkShowKabupaten">Kabupaten</label>
                </div>
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="4" id="chkJudul" checked>
                    <label class="form-check-label" for="chkJudul">Judul</label>
                </div>
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="5" id="chkDeskripsi" checked>
                    <label class="form-check-label" for="chkDeskripsi">Deskripsi</label>
                </div>
            </div>
          </div>
          <div class="modal-footer">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Informasi_terkait/upload_file"); ?>',
    ...
    init: function() {
      ...
      this.on("sendingmultiple", function(data, xhr, formData) {
          formData.append("id_pengawasan", jQuery("#id_row").val());
      });
    },
    success: function (file, response) {      
      <?php if(isset($kontrak_pusat[0])){ ?>
      window.location="<?php echo base_url('Informasi_terkait/index?id='.$kontrak_pusat[0]->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Informasi_terkait'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Informasi_terkait/index_json"); ?>',
        "dataType": "json",
    },
  ...
  function showUploadedFiles(id_pengawasan)
  {
    $.ajax({
      url: "<?php echo base_url('Informasi_terkait/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id_pengawasan},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/laporan_pengawasan/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
            content += "<td><a class='btn btn-s btn-danger' onclick='removeImage("+id_pengawasan+", "+i+")'><i class='glyphicon glyphicon-trash'></i></a></td>";
            content += "</tr>";
          }
  ...
  function removeImage(id_pengawasan, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Informasi_terkait/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_pengawasan : id_pengawasan, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($kontrak_pusat[0])){ ?>
          window.location="<?php echo base_url('Informasi_terkait/index?id='.$kontrak_pusat[0]->id); ?>";
          <?php }else{ ?>
            window.location="<?php echo base_url('Laporan_pengawasan'); ?>";
          <?php } ?>
        },
        error: function(jqXHR, textStatus, errorThrown) {             
          <?php if(isset($kontrak_pusat[0])){ ?>
  ...
  function LoadData(id_pengawasan)
  {
    $.ajax({
      url: "<?php echo base_url('Informasi_terkait/get_json'); ?>",
      dataType: 'JSON',
      ...
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/laporan_pengawasan/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/laporan_pengawasan/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/laporan_pengawasan/"+obj[i], "#embedPDF_"+i);
        ...
        $('#prev_judul').html(data['judul']);
        $('#prev_deskripsi').html(data['deskripsi']);
