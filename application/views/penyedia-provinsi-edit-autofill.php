<div class="col_two_fifth col_centered nobottommargin">
    <div class="well well-lg nobottommargin">
      <form method="post" action="<?php echo base_url('PenyediaProvinsi/update_autofill'); ?>">
        <input type="hidden" name="id" value="<?php echo $penyedia_provinsi->id; ?>">
        <h3>UBAH DATA AUTOFILL PENYEDIA PROVINSI: <br/><?php echo $penyedia_provinsi->nama_penyedia_provinsi ?></h3>
        <hr>
        <div class="col_full">
          <label>Nama Yang Menyerahkan</label>
          <input type="text" name="nama_penyerah" class="form-control" value="<?php echo $penyedia_provinsi->nama_penyerah; ?>" />
        </div>
        <div class="col_full">
          <label>Jabatan Yang Menyerahkan</label>
          <input type="text" name="jabatan_penyerah" class="form-control" value="<?php echo $penyedia_provinsi->jabatan_penyerah; ?>" />
        </div>
        <div class="col_full">
          <label>Nomor Telepon Yang Menyerahkan</label>
          <input type="text" name="notelp_penyerah" class="form-control" value="<?php echo $penyedia_provinsi->notelp_penyerah; ?>" />
        </div>
        <div class="col_full">
          <label>Alamat Yang Menyerahkan</label>
          <textarea rows=3 name="alamat_penyerah" class="form-control"><?php echo $penyedia_provinsi->alamat_penyerah; ?></textarea>
        </div>
        <div class="col_full">
          ...
          <?php 
            foreach($provinsi as $prov){
              echo "<option value=".$prov->id." ".($prov->id == $penyedia_provinsi->id_provinsi_penyerah ? 'selected' : '')." >".$prov->nama_provinsi."</option>";
            }
          ?>
          </select>
        </div>
