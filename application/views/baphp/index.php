...
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  ...
  <span class="sisamasa">(tersisa <?php echo max(round((strtotime($kontrak_pusat->periode_selesai) - time()) / (60 * 60 * 24)),0) ?> hari)</span></h4>
...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp/create?id_alokasi=').$alokasi_pusat->id; ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input BAPHP</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
  ...
  <?php if($grafik == 1) { ?>
  <div class="col-sm-6">
    <div class="col-sm-3" style="width: 120px; height: 100px; text-align: center; padding-top: 15px; border-right: solid; border-right-color: #efefef; ">
...
<div style="margin-top: 150px; ">
  ...
  <thead>
    ...
    <?php
      // only show batitip if Dinas=PUSAT
      if(isset($alokasi_pusat)){
        if($alokasi_pusat->dinas != 'PUSAT' && ($val['column'] == 'no_batitip' || $val['column'] == 'tanggal_batitip')){
          continue;
        }
      }
    ?>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Baphp/upload_file"); ?>',
    ...
    success: function (file, response) {      
      <?php if(isset($kontrak_pusat)){ ?>
      window.location="<?php echo base_url('Baphp/index?id_alokasi='.$alokasi_pusat->id); ?>";
      <?php }else{ ?>
  ...
  Dropzone.options.myDropzonefoto= {
    // The configuration we've talked about above
    url: '<?php echo base_url("Baphp/upload_filefoto"); ?>',
    acceptedFiles: 'image/*,.pdf',
    addRemoveLinks: true,
    autoProcessQueue: false,
    uploadMultiple: true,
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Baphp/index_json"); ?>',
        <?php if(isset($alokasi_pusat)){ ?>
        "data": {
           id_alokasi : '<?php echo $alokasi_pusat->id; ?>',
           regcad : '<?php echo $regcad; ?>',
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  $("#ExportReporttoExcel").on("click", function() {
    $.ajax({
      type: "POST",
      url: '<?php echo base_url("Baphp/export"); ?>',
      <?php if(isset($alokasi_pusat)){ ?>
      data: {id_alokasi : <?php echo $alokasi_pusat->id;?>},
      <?php } ?>
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Baphp/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
      success: function(data) {
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/baphp/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
  ...
  function removeImagefoto(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp/remove_filefoto'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id : id, urutanfile: urutan},
        success: function(data) { 
      ...
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/baphp/"+obj[i], "#embedPDF_"+i);
