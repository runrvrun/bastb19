...
<?php if(isset($kontrak_ongkir)){ ?>
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  <h4><?php echo $kontrak_ongkir->nomor; ?></h4>
  <h2><?php echo $kontrak_ongkir->ekspedisi; ?></h2>
  <h4><?php echo date('d M Y', strtotime($kontrak_ongkir->tanggal)); ?></h4>
  <div class="btn-group" role="group" aria-label="Basic example">
    <button type="button" class="btn btn-default btn-lg" style="width: 300px; height: 50px;">
      ...
      Rp <b><?php echo number_format($kontrak_ongkir->ongkir, 0); ?></b><br>
    </button>
  </div>
</div>
<hr>
...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-2" style="height: 120px; border: solid 1px; ">
  ...
  <div class="col-sm-6" style="width: 110px; text-align: center;">
      <div class="pie_progress" role="progressbar" data-goal="<?php echo($kontrak_ongkir->ongkir == 0 ? 0 : ($total_nilai/$kontrak_ongkir->ongkir*100)); ?>" data-barcolor="#3daf2c" aria-valuemin="0" aria-valuemax="100">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Penggunaan_ongkir/upload_file"); ?>',
    ...
    init: function() {
      ...
      this.on("sendingmultiple", function(data, xhr, formData) {
          formData.append("id_ongkir", jQuery("#id_row").val());
      });
    },
    success: function (file, response) {      
      <?php if(isset($baphp)){ ?>
      window.location="<?php echo base_url('Penggunaan_ongkir/index?id='.$baphp->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Penggunaan_ongkir'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Penggunaan_ongkir/index_json"); ?>',
        <?php if(isset($kontrak_ongkir)){ ?>
        "data": {
           id_kontrak_ongkir : '<?php echo $kontrak_ongkir->id; ?>'
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  function showUploadedFiles(id_ongkir)
  {
    $.ajax({
      url: "<?php echo base_url('Penggunaan_ongkir/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id_ongkir},
      success: function(data) {
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            content += "<td><a class='btn btn-s btn-danger' onclick='removeImage("+id_ongkir+", "+i+")'><i class='glyphicon glyphicon-trash'></i></a></td>";
            content += "</tr>";
          }
  ...
  function removeImage(id_ongkir, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Penggunaan_ongkir/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_ongkir : id_ongkir, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($baphp)){ ?>
          window.location="<?php echo base_url('Penggunaan_ongkir/index?id='.$baphp->id); ?>";
          <?php }else{ ?>
