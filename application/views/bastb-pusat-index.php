...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("BASTBPusat/Test"); ?>',
    ...
    success: function (file, response) {
          ...
          // }
          // alert(response);
          window.location="<?php echo base_url('BASTBPusat'); ?>";
    },
  }
  ...
  $("#ExportReporttoExcel").on("click", function() {
    ...
    $.ajax({
      type: "POST",
      url: '<?php echo site_url("BASTBPusat/doExport"); ?>',
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("BASTBPusat/AjaxGetAllData"); ?>',
        "data": function(d) {
  ...
  {
    $.ajax({
      url: "<?php echo base_url('BASTBPusat/GetBASTB'); ?>",
      dataType: 'JSON',
      type: 'GET',
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='../upload/bastb_pusat/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id_bastb, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('BASTBPusat/RemoveImage'); ?>",
        dataType: 'JSON',
        type: 'GET',
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='../upload/bastb_pusat/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='../upload/bastb_pusat/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("../upload/bastb_pusat/"+obj[i], "#embedPDF_"+i);
