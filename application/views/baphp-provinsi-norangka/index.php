...
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  <h4><?php echo $baphp_provinsi->nama_penyedia_provinsi; ?></h4>
  <h2><?php echo $baphp_provinsi->no_baphp; ?></h2>
  <h4><?php echo date('d M Y', strtotime($baphp_provinsi->tanggal)); ?></h4>
  <div class="btn-group" role="group" aria-label="Basic example">
    <button type="button" class="btn btn-default btn-lg" style="width: 250px; height: 50px;white-space: normal;">
      <!-- <i class="glyphicon glyphicon-cog"></i><br> -->
      <b><?php echo $baphp_provinsi->nama_barang ?></b><br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 250px; height: 50px;white-space: normal;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo $baphp_provinsi->merk; ?></b><br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 200px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo number_format($baphp_provinsi->jumlah_barang, 0); ?></b> UNIT<br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 300px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      Rp <b><?php echo number_format($baphp_provinsi->nilai_barang, 0); ?></b><br>
    </button>
  </div>
</div>
<hr>
...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-4" style="height: 120px; border: solid 1px; ">
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp_provinsi/index?id_alokasi_provinsi='.$baphp_provinsi->id_alokasi_provinsi); ?>">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp_provinsi_norangka/create?id_baphp_provinsi='.$baphp_provinsi->id); ?>">
  ...
  <div style="float: right; margin-right: -30px;">
    <div class="col-sm-6" style="width: 200px; height: 100px; text-align: center; padding-top: 15px;">
      ...
      <b style="font-size: 22px; "><span class="Count"><?php echo count($baphp_provinsi_norangka); ?></span></b><br> Unit
    </div>
  </div>
</div>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    success: function (file, response) {      
      <?php if(isset($baphp_provinsi)){ ?>
      window.location="<?php echo base_url('Norangka/index?id='.$baphp_provinsi->id); ?>";
      <?php }else{ ?>
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Baphp_provinsi_norangka/index_json"); ?>',
        <?php if(isset($baphp_provinsi)){ ?>
        "data": {
           id_baphp_provinsi : '<?php echo $baphp_provinsi->id; ?>'
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Baphp_provinsi_norangka/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id_norangka},
  ...
  function removeImage(id_norangka, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp_provinsi_norangka/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_norangka : id_norangka, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($baphp_provinsi)){ ?>
          window.location="<?php echo base_url('Baphp_provinsi_norangka/index?id='.$baphp_provinsi->id); ?>";
          <?php }else{ ?>
            window.location="<?php echo base_url('Baphp_provinsi_norangka'); ?>";
          <?php } ?>
        },
        error: function(jqXHR, textStatus, errorThrown) {             
          <?php if(isset($baphp_provinsi)){ ?>
