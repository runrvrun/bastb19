...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Bart/create?id_kontrak_pusat='.$kontrak_pusat->id); ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input Bart</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
...
<div class="modal fade" id="upload-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="color:black;">
        <div class="modal-content">
            <div class="modal-header">
                Upload File Untuk Bart <b><i class="title"></i></b>
            </div>
            <div class="modal-body">
              <div class="col_full">
                <label>UPLOAD FOTO/DOKUMEN Bart</label>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Bart/upload_file"); ?>',
    ...
    init: function() {
      ...
      this.on("sendingmultiple", function(data, xhr, formData) {
          formData.append("id_bart", jQuery("#id_row").val());
      });
    },
    success: function (file, response) {      
      <?php if(isset($kontrak_pusat)){ ?>
      window.location="<?php echo base_url('Bart/index?id_kontrak_pusat='.$kontrak_pusat->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Bart'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Bart/index_json"); ?>',
        <?php if(isset($kontrak_pusat)){ ?>
        "data": {
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Bart/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/bart/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id_bart, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Bart/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_bart : id_bart, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($kontrak_pusat)){ ?>
          window.location="<?php echo base_url('Bart/index?id_kontrak_pusat='.$kontrak_pusat->id); ?>";
          <?php }else{ ?>
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/bart/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/bart/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/bart/"+obj[i], "#embedPDF_"+i);
