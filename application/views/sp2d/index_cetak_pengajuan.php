...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Sp2d/index?id_kontrak_pusat=').$kontrak_pusat->id; ?>">
        <img src="<?php echo base_url('assets/ico/back_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Kembali</span>
    </div>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Sp2d/cetak_pengajuan_pdf?id_kontrak_pusat='.$kontrak_pusat->id.'&keterangan='.$keterangan); ?>">
      <img style="cursor: pointer;" src="<?php echo base_url('assets/ico/print.png'); ?>" width="50px" />
      <br>
      <span style="font-size:10px; font-weight: bold;">Cetak Berkas</span>
      </a>
    </div>
  </a>
  </div>
...
<div class="modal fade" id="upload-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="color:black;">
        <div class="modal-content">
            <div class="modal-header">
                Upload File Untuk SP2D <b><i class="title"></i></b>
            </div>
            <div class="modal-body">
              <div class="col_full">
                <label>UPLOAD FOTO/DOKUMEN SP2D</label>
      ...
      <div class="modal-content">
          ...
          <div class="modal-body" style="width: 1000px; ">
            <div class="row">
                ...
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="3" id="chkJudul" checked>
                    <label class="form-check-label" for="chkJudul">Judul</label>
                </div>
                <div class="col-md-3">
                    <input class="form-check-input" name="show" type="checkbox" value="3" id="chkDeskripsi" checked>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Sp2d/upload_file"); ?>',
    ...
    init: function() {
      ...
      this.on("sendingmultiple", function(data, xhr, formData) {
          formData.append("id_sp2d", jQuery("#id_row").val());
      });
    },
    success: function (file, response) {      
      <?php if(isset($kontrak_pusat)){ ?>
      window.location="<?php echo base_url('Sp2d/index?id='.$kontrak_pusat->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Sp2d'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Sp2d/cetak_pengajuan_json"); ?>',
        <?php if(isset($kontrak_pusat)){ ?>
        "data": {
  ...
  function showUploadedFiles(id_sp2d)
  {
    $.ajax({
      url: "<?php echo base_url('Sp2d/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id_sp2d},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='../../upload/sop/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
            content += "<td><a class='btn btn-s btn-danger' onclick='removeImage("+id_sp2d+", "+i+")'><i class='glyphicon glyphicon-trash'></i></a></td>";
            content += "</tr>";
          }
  ...
  function removeImage(id_sp2d, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Sp2d/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_sp2d : id_sp2d, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($kontrak_pusat)){ ?>
          window.location="<?php echo base_url('Sp2d/index?id='.$kontrak_pusat->id); ?>";
          <?php }else{ ?>
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='../../upload/sop/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='../../upload/sop/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("../../upload/sop/"+obj[i], "#embedPDF_"+i);
