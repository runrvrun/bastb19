...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-4" style="height: 120px; border: solid 1px; ">
    ...
    <div class="col-sm-4" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Kontrak_pusat/create'); ?>">
...
<div style="margin-top: 150px; ">
  ...
  <thead>
    <tr>
      ...
      <td>
        <select class="search-input-select" data-column="11">
          <option value=""></option>
  ...
  <tbody>
    <?php 
      for($i=0; $i<count($kontrak_pusat); $i++){
        echo "<tr>
                <td>".$kontrak_pusat[$i]->tahun_anggaran."</td>
                <td>".$kontrak_pusat[$i]->nama_penyedia_pusat."</td>
                <td>".$kontrak_pusat[$i]->no_kontrak."</td>
                <td>".date('d-m-Y', strtotime($kontrak_pusat[$i]->periode_mulai))." s/d ".date('d-m-Y', strtotime($kontrak_pusat[$i]->periode_selesai))."</td>
                <td>".$kontrak_pusat[$i]->nama_barang."</td>
                <td>".$kontrak_pusat[$i]->merk."</td>
                <td>".number_format($kontrak_pusat[$i]->jumlah_barang, 0)."</td>
                <td>".number_format($kontrak_pusat[$i]->nilai_barang, 0)."</td>
                <td>".number_format(($kontrak_pusat[$i]->nilai_barang / $kontrak_pusat[$i]->jumlah_barang), 0)."</td>
                <td>".number_format($kontrak_pusat[$i]->jumlah_alokasi, 0)."</td>
                <td>".number_format($kontrak_pusat[$i]->nilai_alokasi, 0)."</td>
                <td>".(($kontrak_pusat[$i]->jumlah_barang - $kontrak_pusat[$i]->jumlah_alokasi == 0 and $kontrak_pusat[$i]->nilai_barang - $kontrak_pusat[$i]->nilai_alokasi == 0) ? 'CUKUP' : 'KURANG')."</td>
                <td>".($kontrak_pusat[$i]->nama_file != '' ?  'TERSEDIA' : 'TDK TERSEDIA')."</td>
                <td nowrap>";
    ?>
              <a class="btn btn-xs btn-warning btn-sm" href="<?php echo base_url('Kontrak_pusat/ShowAlokasi?id=').$kontrak_pusat[$i]->id; ?>"><i class="glyphicon glyphicon-plus"></i></a>
              <a class="btn btn-xs btn-success btn-sm"><i class="glyphicon glyphicon-zoom-in" onclick="LoadData(<?php echo $kontrak_pusat[$i]->id; ?>);"></i></a>
              <a class="btn btn-xs btn-primary btn-sm" href="<?php echo base_url('Kontrak_pusat/Edit?id=').$kontrak_pusat[$i]->id; ?>"><i class="glyphicon glyphicon-pencil"></i></a>
              <a class="btn btn-xs btn-danger btn-sm" data-href="<?php echo base_url('Kontrak_pusat/doDelete?id=').$kontrak_pusat[$i]->id; ?>" data-toggle="modal" data-record-title="<?php echo $kontrak_pusat[$i]->no_kontrak; ?>" data-target="#confirm-delete"><i class="glyphicon glyphicon-trash"></i></a>
              <a class="btn btn-xs btn-info btn-sm" data-href="#" data-toggle="modal" data-record-title="<?php echo $kontrak_pusat[$i]->no_kontrak; ?>" data-target="#upload-modal" data-record-id="<?php echo $kontrak_pusat[$i]->id; ?>"><i class="glyphicon glyphicon-open-file"></i></a>
    <?php
        echo "</td>
              </tr>";
      }
...
<!--  <div class="img-zoom-container">
  <img id="myimage" src="<?php echo base_url('<?php echo base_url();?>/upload/kontrak_pusat/frenchie.jpg'); ?>" width="250px" />
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Kontrak_pusat/upload_file"); ?>',
    ...
    init: function() {
      ...
      this.on("sendingmultiple", function(data, xhr, formData) {
          formData.append("id_kontrak_pusat", jQuery("#id_row").val());
      });
    },
    success: function (file, response) {
          window.location="<?php echo base_url('Kontrak_pusat'); ?>";
    },
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Kontrak_pusat/index_json"); ?>',
        "dataType": "json",
    },
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Kontrak_pusat/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/kontrak_pusat/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id_kontrak, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Kontrak_pusat/remove_image'); ?>",
        dataType: 'JSON',
        type: 'GET',
  ...
  function LoadData(id)
  {
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/kontrak_pusat/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/kontrak_pusat/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/kontrak_pusat/"+obj[i], "#embedPDF_"+i);
        ...
        $('#prev_nama_penyedia').html(data['nama_penyedia_pusat']);
