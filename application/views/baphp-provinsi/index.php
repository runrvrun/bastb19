...
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  ...
  (tersisa <?php echo max(round((strtotime($kontrak_provinsi->periode_selesai) - time()) / (60 * 60 * 24)),0) ?> hari)</h4>
  <div class="btn-group" role="group" aria-label="Basic example">
    ...
    <button type="button" class="btn btn-default btn-lg" style="width: 200px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo number_format($alokasi_provinsi->jumlah_barang_rev, 0); ?></b> UNIT<br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 300px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      Rp <b><?php echo number_format($alokasi_provinsi->nilai_barang_rev, 0); ?></b><br>
    </button>
  </div>
</div>
<hr>
...
<div class="col-sm-12 npl npr mb20 dt-buttons">
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Alokasi_provinsi/index_confirmed'); ?>">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp_provinsi/create?id_alokasi_provinsi=').$alokasi_provinsi->id; ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input BAPHP Provinsi</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
...
<div class="col-sm-12 npl npr mb20 dt-buttons">
  ...
  <div class="col-sm-8">
    ...
    <div class="col-sm-3" style="min-width: 120px; height: 150px; text-align: center; border-right: solid; border-right-color: #efefef; ">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Baphp_provinsi/upload_file"); ?>',
    ...
    success: function (file, response) {      
      ...
      window.location="<?php echo base_url('Baphp_provinsi/index?id_alokasi_provinsi='.$alokasi_provinsi->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Baphp'); ?>";
      <?php } ?>
    }
  }
  ...
  Dropzone.options.myDropzonefoto= {
    // The configuration we've talked about above
    url: '<?php echo base_url("Baphp_provinsi/upload_filefoto"); ?>',
    acceptedFiles: 'image/*,.pdf',
    addRemoveLinks: true,
    autoProcessQueue: false,
    uploadMultiple: true,
    ...
    success: function (file, response) {      
      ...
      window.location="<?php echo base_url('Baphp_provinsi/index?id_alokasi_provinsi='.$alokasi_provinsi->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Baphp_provinsi'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Baphp_provinsi/index_json"); ?>',
        <?php if(isset($alokasi_provinsi)){ ?>
        "data": {
          id_alokasi_provinsi : '<?php echo $alokasi_provinsi->id; ?>'
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  $("#ExportReporttoExcel").on("click", function() {
    $.ajax({
      type: "POST",
      url: '<?php echo base_url("Baphp_provinsi/export"); ?>',
      <?php if(isset($baphp_provinsi)){ ?>
      data: {id_alokasi : <?php echo $baphp_provinsi->id_alokasi_provinsi;?>},
      <?php } ?>
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Baphp_provinsi/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/baphp_provinsi/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp_provinsi/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
...
$('#chkStandar').change(function(){
    if($(this).prop("checked") == true){
      ...
      $('#chkShowtitik_serah').prop("checked", true);
      $('#chkShownama_wilayah').prop("checked", true);
      $('#chkShowno_baphp').prop("checked", true);
  ...
  function removeImagefoto(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp_provinsi/remove_filefoto'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id : id, urutanfile: urutan},
        success: function(data) { 
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/baphp_provinsi/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/baphp_provinsi/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/baphp_provinsi/"+obj[i], "#embedPDF_"+i);
