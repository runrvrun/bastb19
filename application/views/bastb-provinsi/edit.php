<div class="col_two_fifth col_centered nobottommargin">
  <div class="well well-lg nobottommargin">
    <form method="post" action="<?php echo base_url('Bastb_provinsi/update'); ?>" onkeypress="return event.keyCode != 13;" style="font-color:black;" enctype="multipart/form-data">
      ...
      <div class="col_full">
        ...
        <?php if($val['column'] == 'kelompok_penerima'){ ?>
          <select class="form-control" name="<?php echo $val['column'];?>">
        ...
        <?php }elseif($val['column'] == 'tanggal') { ?>
          ...
          <input type="text" class="input-sm form-control" name="tanggal" value="<?php echo ($bastb_provinsi->tanggal > '1900-01-01')? date('d-m-Y', strtotime($bastb_provinsi->tanggal)):''; ?>" autocomplete="off" />
        ...
        <?php }elseif($val['column'] == 'provinsi_penerima'){ ?>
          <input type="hidden" name="id_provinsi_penerima" id="id_provinsi_penerima" value="<?php echo $bastb_provinsi->id_provinsi_penerima;?>" />                   
          <input type="text" class="form-control" value="<?php echo $bastb_provinsi->provinsi_penerima;?>" readonly />                   
        ...
        <?php }elseif($val['column'] == 'nama_kecamatan'){ ?>
          <select id="id_kecamatan_penerima" name="id_kecamatan_penerima" class="form-control js-example-basic-single" >
          </select>
        <?php }elseif($val['column'] == 'nama_kelurahan'){ ?>
          <select id="id_kelurahan_penerima" name="id_kelurahan_penerima" class="form-control js-example-basic-single" >
          </select>
        <?php }elseif($val['column'] == 'nama_barang'){ ?>
          <input type="text" name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control" value="<?php echo $bastb_provinsi->{$val['column']};?>" readonly />                   
        <?php }elseif($val['column'] == 'merk'){ ?>
          <input type="text" name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control" value="<?php echo $bastb_provinsi->{$val['column']};?>" readonly />                   
        <?php }elseif($val['column'] == 'nilai_barang'){ ?>
          <input type="text" name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control" value="<?php echo number_format($bastb_provinsi->nilai_barang,0);?>" readonly />                   
        ...
        <?php }elseif($val['column'] == 'alamat_penyerah'){ ?>
          <textarea rows=3 name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control"><?php echo $bastb_provinsi->{$val['column']};?></textarea>                   
        <?php }elseif($val['column'] == 'alamat_penerima'){ ?>
          <textarea rows=3 name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control"><?php echo $bastb_provinsi->{$val['column']};?></textarea>                   
        <?php }elseif($val['column'] == 'provinsi_penyerah'){ ?>
          <input type="hidden" name="id_provinsi_penyerah" value="<?php echo $alokasi_provinsi->id_provinsi;?>"/>
        ...
        <?php }elseif($val['column'] == 'kabupaten_penyerah'){ ?>
          <input type="hidden" name="id_kabupaten_penyerah" value="<?php echo $alokasi_provinsi->id_kabupaten;?>"/>
          <select id="id_kabupaten_penyerah" class="form-control js-example-basic-single" disabled>
          </select>
        <?php }else{ ?>
          <input type="text" name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control" value="<?php echo $bastb_provinsi->{$val['column']} ?>" />
        <?php } ?>
      </div>
      <?php } ?>
      <div class="col_full nobottommargin">
        <!-- <a href="<?php echo base_url('Bastb_provinsi/index?id_alokasi=').$alokasi_provinsi->id; ?>"> -->
  ...
  function LoadKelurahan()
  {
    $.ajax({
      ...
      success: function(data) {
          ...
          for(var i=0;i<data.length;i++)
          {
            if(data[i]["id"] == '<?php echo (empty($bastb_provinsi->id_kelurahan_penerima))? 0:$bastb_provinsi->id_kelurahan_penerima; ?>'){
