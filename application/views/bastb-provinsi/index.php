...
<div class="col-sm-12 npl npr mb20 dt-buttons">
  ...
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Bastb_provinsi/create?id_alokasi=').$alokasi_provinsi->id; ?>">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Bastb_provinsi/upload_file"); ?>',
    ...
    success: function (file, response) {      
      <?php if(isset($alokasi_provinsi)){ ?>
      window.location="<?php echo base_url('Bastb_provinsi/index?id_alokasi_provinsi='.$alokasi_provinsi->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Bastb_provinsi'); ?>";
      <?php } ?>
    }
  }
  ...
  Dropzone.options.myDropzonefoto= {
    // The configuration we've talked about above
    url: '<?php echo base_url("Bastb_provinsi/upload_filefoto"); ?>',
    acceptedFiles: 'image/*,.pdf',
    addRemoveLinks: true,
    autoProcessQueue: false,
    uploadMultiple: true,
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Bastb_provinsi/index_json"); ?>',
        <?php if(isset($alokasi_provinsi)){ ?>
        "data": {
           id_alokasi : '<?php echo $alokasi_provinsi->id; ?>',
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  $("#ExportReporttoExcel").on("click", function() {
    $.ajax({
      type: "POST",
      url: '<?php echo base_url("Bastb_provinsi/export"); ?>',
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Bastb_provinsi/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
      success: function(data) {
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/bastb_provinsi/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Bastb_provinsi/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
  ...
  function removeImagefoto(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Bastb_provinsi/remove_filefoto'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id : id, urutanfile: urutan},
        success: function(data) { 
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/bastb_provinsi/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/bastb_provinsi/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/bastb_provinsi/"+obj[i], "#embedPDF_"+i);
