...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("HibahPusat/Test"); ?>',
    ...
    init: function() {
      ...
      this.on("sendingmultiple", function(data, xhr, formData) {
          formData.append("id_hibah_pusat", jQuery("#id_row").val());
      });
    },
    success: function (file, response) {
      
          window.location="<?php echo base_url('HibahPusat'); ?>";
    },
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("HibahPusat/AjaxGetAllData"); ?>',
        "dataType": "json",
    },
    columns: [
        { data: "tahun_anggaran" },
  ...
  function showUploadedFiles(id_hibah_pusat)
  {
    $.ajax({
      url: "<?php echo base_url('HibahPusat/GetHibah'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id_hibah_pusat : id_hibah_pusat},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='../upload/pdflogo.png'/></td><td><a href='../upload/hibah_pusat/"+obj[i]+"' download>"+obj[i].substring(10)+"</a></td>";
            }
            else{
              content += "<td>"+(i+1)+"</td><td><img src='../upload/hibah_pusat/"+obj[i]+"'/></td><td><a href='../upload/hibah_pusat/"+obj[i]+"' download>"+obj[i].substring(10)+"</a></td>";
            }
            content += "<td><a class='btn btn-s btn-danger' onclick='removeImage("+id_hibah_pusat+", "+i+")'><i class='glyphicon glyphicon-trash'></i></a></td>";
            content += "</tr>";
          }
  ...
  function removeImage(id_hibah_pusat, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('HibahPusat/RemoveImage'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id_hibah_pusat : id_hibah_pusat, urutanfile: urutan},
        success: function(data) {
  ...
  function LoadData(id_hibah_pusat)
  {
    $.ajax({
      url: "<?php echo base_url('HibahPusat/GetHibah'); ?>",
      dataType: 'JSON',
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='../upload/hibah_pusat/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='../upload/hibah_pusat/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("../upload/hibah_pusat/"+obj[i], "#embedPDF_"+i);
