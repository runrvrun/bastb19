...
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  <h4><?php echo $baphp_persediaan->nama_penyedia_pusat; ?></h4>
  <h2><?php echo $baphp_persediaan->no_baphp; ?></h2>
  <h4><?php echo date('d M Y', strtotime($baphp_persediaan->tanggal)); ?></h4>
  <div class="btn-group" role="group" aria-label="Basic example">
    <button type="button" class="btn btn-default btn-lg" style="width: 250px; height: 50px;white-space: normal;">
      <!-- <i class="glyphicon glyphicon-cog"></i><br> -->
      <b><?php echo $baphp_persediaan->nama_barang ?></b><br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 250px; height: 50px;white-space: normal;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo $baphp_persediaan->merk; ?></b><br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 200px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo number_format($baphp_persediaan->jumlah_barang, 0); ?></b> UNIT<br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 300px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      Rp <b><?php echo number_format($baphp_persediaan->nilai_barang, 0); ?></b><br>
    </button>
  </div>
</div>
<hr>
...
<div class="col-md-12 npl npr mb20 dt-buttons">
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp_persediaan/index?id_alokasi_persediaan='.$baphp_persediaan->id_alokasi_persediaan_pusat); ?>">
        <img src="<?php echo base_url('assets/ico/back_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Kembali Ke BAPHP</span>
    </div>
    <?php if($bolehtambah) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Ongkir_persediaan/create?id_baphp_persediaan='.$baphp_persediaan->id); ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input Ongkir</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
...
<div class="modal fade" id="upload-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="color:black;">
        <div class="modal-content">
            <div class="modal-header">
                Upload File Untuk Data Ongkir <b><i class="title"></i></b>
            </div>
            <div class="modal-body">
              <div class="col_full">
                <label>UPLOAD FOTO/DOKUMEN Data Ongkir</label>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Ongkir_persediaan/upload_file"); ?>',
    ...
    success: function (file, response) {      
      <?php if(isset($baphp_persediaan)){ ?>
      window.location="<?php echo base_url('Ongkir_persediaan/index?id_baphp_persediaan='.$baphp_persediaan->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Ongkir_persediaan'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Ongkir_persediaan/index_json"); ?>',
        <?php if(isset($baphp_persediaan)){ ?>
        "data": {
           id_baphp_persediaan : '<?php echo $baphp_persediaan->id; ?>'
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Ongkir_persediaan/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/ongkir_persediaan/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Ongkir_persediaan/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id : id, urutanfile: urutan},
        success: function(data) { 
          <?php if(isset($baphp_persediaan)){ ?>
          window.location="<?php echo base_url('Ongkir_persediaan/index?id_baphp='.$baphp_persediaan->id); ?>";
          <?php }else{ ?>
            window.location="<?php echo base_url('Ongkir_persediaan'); ?>";
          <?php } ?>
        },
  ...
  function LoadData(id)
  {
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/ongkir_persediaan/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/ongkir_persediaan/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/ongkir_persediaan/"+obj[i], "#embedPDF_"+i);
        ...
        <?php } ?>
        $('#prev_id_kontrak_ongkir').html(data['nomor']);
