<div class="col_two_fifth col_centered nobottommargin">
  <div class="well well-lg nobottommargin">
    <form method="post" action="<?php echo base_url('Ongkir_persediaan/store'); ?>" style="font-color:black;">
      ...
      <div class="col_full">
        ...
        <?php if(in_array($val['column'], array('tanggal_surat_permohonan','tanggal_surat_ke_penyedia','tanggal_surat_ke_dinas'))) { ?>
          ...
          <input type="text" class="input-sm form-control" name="<?php echo $val['column'];?>" required autocomplete="off" />
        ...
        <?php }elseif($val['column'] == 'id_kontrak_ongkir') { ?>
          <select name="<?php echo $val['column'];?>" class="form-control">
            <?php foreach($kontrak_ongkir as $key=>$val){?>
              <option value='<?php echo $val->id;?>'><?php echo $val->nomor;?> - Rp <?php echo number_format($val->ongkir,0);?></option>
            <?php } ?>
          </select>
        ...
        <?php }elseif($val['column'] == 'keterangan') { ?>
          <textarea name="<?php echo $val['column'];?>" id="<?php echo $val['column'];?>" class="form-control"></textarea>
        <?php }else{ ?>
      ...
      <div class="col_full nobottommargin">
        <a href="<?php echo base_url('Ongkir_persediaan/index?id_baphp_persediaan='.$id_baphp_persediaan); ?>">
