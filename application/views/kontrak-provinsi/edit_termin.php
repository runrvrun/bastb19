<div class="col_two_fifth col_centered nobottommargin">
  <div class="well well-lg nobottommargin">
    <form method="post" action="<?php echo base_url('Kontrak_provinsi/update_termin'); ?>" onkeypress="return event.keyCode != 13;" style="font-color:black;">
      ...
      <div class="col_full">
        ...
        <select name='jumlah_termin' id='jumlah_termin' class="form-control">
          <option value=1 <?php echo ($kontrak_provinsi->jumlah_termin == 1)? 'selected':'' ?>>1</option>
          <option value=2 <?php echo ($kontrak_provinsi->jumlah_termin == 2)? 'selected':'' ?>>2</option>
          <option value=3 <?php echo ($kontrak_provinsi->jumlah_termin == 3)? 'selected':'' ?>>3</option>
          <option value=4 <?php echo ($kontrak_provinsi->jumlah_termin == 4)? 'selected':'' ?>>4</option>
          <option value=5 <?php echo ($kontrak_provinsi->jumlah_termin == 5)? 'selected':'' ?>>5</option>
        </select>
      </div>
      <?php foreach($cols as $key=>$val){ ?>
      <div class="col_full" id='input_<?php echo $val['column'];?>'>
        <label><?php echo $val['caption'];?></label>
        <input type="text" name="<?php echo $val['column'];?>" class="form-control numberFilter" value="<?php echo $kontrak_provinsi->{$val['column']}; ?>" />
      </div>
      <?php } ?>
