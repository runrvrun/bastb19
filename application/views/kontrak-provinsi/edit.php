<div class="col_two_fifth col_centered nobottommargin">
  <div class="well well-lg nobottommargin">
    <form method="post" action="<?php echo base_url('Kontrak_provinsi/update'); ?>" onkeypress="return event.keyCode != 13;" style="font-color:black;" enctype="multipart/form-data">
      <input type="hidden" name="id" value="<?php echo $kontrak_provinsi->id; ?>" />
      <h3>UBAH DATA KONTRAK PROVINSI</h3>
      <hr>
      <div class="col_full">
        <label>Provinsi</label>
        <?php if(isset($this->session->userdata('logged_in')->id_provinsi)){ ?>
        <input type="text" name="nama_provinsi" id="nama_provinsi" class="form-control" value="<?php echo $this->session->userdata('autofill')->nama_provinsi;?>" disabled/>
        <?php }else{ ?>
        <select name="id_provinsi" id="id_provinsi" class="form-control js-example-basic-single">
        <?php 
          foreach($provinsi as $prov){
            echo "<option value=".$prov->id." ";
            echo ($prov->id == $kontrak_provinsi->id_provinsi)? "selected":"";
      ...
      <div class="col_full">
        ...
        <?php 
          foreach($penyedia_provinsi as $penyedia){
            echo "<option value=".$penyedia->id;
            echo ($penyedia->id == $kontrak_provinsi->id_penyedia_provinsi)? " selected":"";
            echo ">".$penyedia->nama_penyedia_provinsi."</option>";
          }
        ?>
        </select>
      </div>
      ...
      <div class="col_full">
        ...
        <!-- <img id="image_preview" src="<?php echo base_url('upload/kontrak_provinsi/').$kontrak_provinsi->nama_file; ?>" alt="Preview Image" /> -->
        <?php
          // if($kontrak_provinsi->nama_file != '' and $kontrak_provinsi->nama_file != '[]' and !is_null($kontrak_provinsi->nama_file) and $kontrak_provinsi->nama_file != 'null' and $kontrak_provinsi->nama_file != null){
          //   $files = json_decode($kontrak_provinsi->nama_file);
          //   echo '<table border="2">';
          //   $i=1;
          ...
          //         <a href="'.base_url('../upload/kontrak_provinsi/').$file.'" target="_blank">'.$file.'</a>
          //       </td>';
          //     }
          //     else{
          //       echo '<td><img src="'.base_url('../upload/kontrak_provinsi/').$file.'" alt="Preview Image" /></td>';
          //     }
