...
<!-- end loading div -->
<?php if(isset($alokasi_persediaan_pusat)){ ?>
<div id="headerKontrak" style="width:100%; text-align:center; border-style: solid; mergin-bottom: 40px">
  ...
  <div class="btn-group" role="group" aria-label="Basic example">
    ...
    <button type="button" class="btn btn-default btn-lg" style="width: 200px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      <b><?php echo number_format($alokasi_persediaan_pusat->jumlah_barang, 0); ?></b> UNIT<br>
    </button>
    <button type="button" class="btn btn-default btn-lg" style="width: 300px; height: 50px;">
      <!-- <i class="glyphicon glyphicon-usd"></i><br> -->
      Rp <b><?php echo number_format($alokasi_persediaan_pusat->nilai_barang, 0); ?></b><br>
    </button>
  </div>
</div>
<hr>
...
<div class="col-sm-12 npl npr mb20 dt-buttons">
  <div class="col-sm-6" style="height: 120px; border: solid 1px; ">
    ...
    <?php if($bolehtambah) { ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
      <a href="<?php echo base_url('Baphp_persediaan/create?id_alokasi_persediaan=').$alokasi_persediaan_pusat->id; ?>">
        <img src="<?php echo base_url('assets/ico/file_icon.png'); ?>" width="50px" />
      </a><br>
      <span style="font-size:10px; font-weight: bold;">Input BAPHP Persediaan</span>
    </div>
    <?php } ?>
    <div class="col-sm-3" style="padding-top: 10px; text-align: center; ">
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Baphp_persediaan/upload_file"); ?>',
    ...
    success: function (file, response) {      
      ...
      window.location="<?php echo base_url('Baphp_persediaan/index?id_alokasi_persediaan='.$alokasi_persediaan_pusat->id); ?>";
      <?php }else{ ?>
  ...
  Dropzone.options.myDropzonefoto= {
    // The configuration we've talked about above
    url: '<?php echo base_url("Baphp_persediaan/upload_filefoto"); ?>',
    acceptedFiles: 'image/*,.pdf',
    addRemoveLinks: true,
    autoProcessQueue: false,
    uploadMultiple: true,
    ...
    success: function (file, response) {      
      ...
      window.location="<?php echo base_url('Baphp_persediaan/index?id_alokasi_persediaan='.$alokasi_persediaan_pusat->id); ?>";
      <?php }else{ ?>
        window.location="<?php echo base_url('Baphp_persediaan'); ?>";
      <?php } ?>
    }
  }
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Baphp_persediaan/index_json"); ?>',
        <?php if(isset($alokasi_persediaan_pusat)){ ?>
        "data": {
          id_alokasi_persediaan : '<?php echo $alokasi_persediaan_pusat->id; ?>'
        },
        <?php } ?>
        "dataType": "json",
    },
  ...
  $("#ExportReporttoExcel").on("click", function() {
    $.ajax({
      type: "POST",
      url: '<?php echo base_url("Baphp_persediaan/export"); ?>',
      <?php if(isset($baphp_persediaan)){ ?>
      data: {id_alokasi : <?php echo $baphp_persediaan->id_alokasi_persediaan;?>},
      <?php } ?>
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Baphp_persediaan/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
      success: function(data) {
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='<?php echo base_url();?>/upload/baphp_persediaan/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
  ...
  function removeImage(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp_persediaan/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
...
$('#chkStandar').change(function(){
  ...
  function removeImagefoto(id, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Baphp_persediaan/remove_filefoto'); ?>",
        dataType: 'JSON',
        type: 'GET',
        data: {id : id, urutanfile: urutan},
        success: function(data) { 
        ...
        if(data['nama_file'] != '' && data['nama_file'] != null ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='<?php echo base_url();?>/upload/baphp_persediaan/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='<?php echo base_url();?>/upload/baphp_persediaan/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("<?php echo base_url();?>/upload/baphp_persediaan/"+obj[i], "#embedPDF_"+i);
