...
<ul class="nav nav-pills">
  <li><a href="Rating_penyedia">Rating Penyedia</a></li>
  <li class="active"><a href="Rating_produk">Rating Produk</a></li>
</ul>
              ...
              <div class="row">
                <div class="col-sm-4">
                  <div style="text-align:center" id="slideshow_thumb">
                    ...
                    <div style="font-size: 1.2em;" id="prev_nama_jenis_barang_pusat"></div>
                ...
                <table class="table table-striped">
                  <?php foreach($colsi as $key=>$val){ ?>
                    <?php if(($val['column'] != 'overall') && ($val['column'] != 'nama_barang')) { ?>
                    <tr>
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    url: '<?php echo base_url("Rating_produk/upload_file"); ?>',
    ...
    success: function (file, response) {      
      ...
      window.location="<?php echo base_url('Rating_produk/index?id='.$kontrak_pusat[0]->id); ?>";
      <?php }else{ ?>
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Rating_produk/index_json"); ?>',
        "dataType": "json",
    },
  ...
  {
    $.ajax({
      url: "<?php echo base_url('Rating_produk/get_json'); ?>",
      dataType: 'JSON',
      type: 'GET',
      data: {id : id},
  ...
  function removeImage(id_sop, urutan)
  {
    ...
    if (r == true) {
      $.ajax({
        url: "<?php echo base_url('Rating_produk/remove_file'); ?>",
        dataType: 'JSON',
        type: 'GET',
  ...
  function LoadData(id_sop)
  {
    $.ajax({
      ...
      success: function(data) {
        ...
        $('#prev_nama_jenis_barang_pusat').html(data['nama_jenis_barang_pusat']);
        $('#prev_overall').rateit('value', data['overall']);
        $('#prev_mutu').rateit('value', data['mutu']);
        $('#prev_daya_tahan').rateit('value', data['daya_tahan']);
        $('#prev_kesesuaian').rateit('value', data['kesesuaian']);
        $('#prev_ketersediaan_suku_cadang').rateit('value', data['ketersediaan_suku_cadang']);
        $('#prev_perawatan').rateit('value', data['perawatan']);
        $('#prev_cara_pengoperasian').rateit('value', data['cara_pengoperasian']);
