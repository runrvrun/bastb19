...
<div class="col-md-12 npl npr mb20 dt-buttons">
  ...
  <div style="float: right; margin-right: -50px;">
    ...
    <?php if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN KABUPATEN'))){  ?>
...
<div style="float:right; margin-bottom:7px;">
  <form class="form-inline" method="POST" action="<?php echo base_url('Alokasi_provinsi/index_confirmed');?>">
...
<div style="">
  ...
  <thead>
    ...
    <?php foreach($cols as $key=>$val){ ?>
      <?php 
        // hide from admin kabupaten
        if(in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN KABUPATEN'))){  
      ...
      <?php } ?>
    <?php } ?>
      <?php if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN KABUPATEN'))){// hide from admin kabupaten ?>
        <td></td>
      <?php } ?>
      <?php if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PENYEDIA KABUPATEN','ADMIN PENYEDIA PROVINSI'))){// hide from admin kabupaten ?>
        <td></td>
      <?php } ?>
      <td></td>
    </tr>
              ...
              <div class="row">
                ...
                <table class="table table-striped">
                  <?php foreach($cols as $key=>$val){ ?>
                    ...
                    ?>
                  <?php 
                      if(in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN KABUPATEN'))){// hide from kabupaten 
                          if($val['column'] == 'status_baphp') continue;
                      }
                      if(in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PENYEDIA KABUPATEN','ADMIN PENYEDIA PROVINSI'))){// hide from admin kabupaten 
      ...
      <div class="modal-content">
          ...
          <div class="modal-body" style="width: 1000px; ">
            ...
            <?php foreach($cols as $key=>$val){ ?>
              ...
              ?>
              <?php 
                      if(in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN KABUPATEN'))){// hide from admin kabupaten 
                          if($val['column'] == 'status_baphp') continue;
                      }
                      if(in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PENYEDIA KABUPATEN','ADMIN PENYEDIA PROVINSI'))){// hide from admin kabupaten 
                        if($val['column'] == 'status_bastb') continue;
...
<script>
  ...
  Dropzone.options.myDropzone= {
    ...
    success: function (file, response) {      
      <?php if(isset($kontrak_provinsi)){ ?>
      window.location="<?php echo base_url('Alokasi_provinsi/index?id='.$kontrak_provinsi->id); ?>";
      <?php }else{ ?>
  ...
  var table = $('#Table1').DataTable({
    ...
    "ajax": {
        "type": "POST",
        "url": '<?php echo base_url("Alokasi_provinsi/index_confirmed_json"); ?>',
        <?php if(isset($kontrak_provinsi)){ ?>
        "data": {
  ...
  $("#ExportReporttoExcel").on("click", function() {
    $.ajax({
      type: "POST",
      url: '<?php echo base_url("Alokasi_provinsi/export_confirmed"); ?>',
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'PDF' || file_format == 'pdf'){
              ...
              content += "<td>"+(i+1)+"</td><td><img src='../../upload/alokasi_provinsi/"+obj[i]+"'/></td><td>"+obj[i].substring(10)+"</td>";
            }
        ...
        if(data['nama_file'] != '' && data['nama_file'] != "null" ){
          ...
          for(var i=0;i<obj.length;i++)
          {
            ...
            if(file_format == 'pdf' || file_format == 'PDF'){
              ...
              content += "<div class='mySlides'><div class='numbertext'> "+(i+1)+" / "+obj.length+" </div><img id='zoom_"+i+"' src='../../upload/alokasi_provinsi/"+obj[i]+"' height='300' /><div class='text'>"+obj[i].substring(10)+"&nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-primary btn-sm' href='../../upload/alokasi_provinsi/"+obj[i]+"' download><i class='glyphicon glyphicon-save-file'></i></a></div></div>";
            }
          ...
          for(var i=0;i<obj.length;i++)
          {
            var file_format = obj[i].substr(obj[i].length - 3); 
            if(file_format == 'pdf' || file_format == 'PDF'){
              PDFObject.embed("../../upload/alokasi_provinsi/"+obj[i], "#embedPDF_"+i);
