...
class BAPSTHPReguler extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('BAPSTHPRegulerModel');
	...
	public function index()
	{
		...
		$param['bapsthp_reguler'] = array();
		$param['total_unit'] = $this->BAPSTHPRegulerModel->GetTotalUnit();
		$param['total_nilai'] = $this->BAPSTHPRegulerModel->GetTotalNilai();
		...
		$data = array(
	        'title' => 'BAP-STHP Reguler',
	        'content-path' => 'PENGADAAN PUSAT / BAP-STHP REGULER',
	        'content' => $this->load->view('bapsthp-reguler-index', $param, TRUE),
		);
	...
	public function AjaxGetAllData()
	{
		...
		$totalData = $this->BAPSTHPRegulerModel->GetAllAjaxCount();
        ...
        {            
            $posts = $this->BAPSTHPRegulerModel->GetAllForAjax($_POST['start'], $_POST['length'], $order, $dir, $filtercond);
            $totalFiltered = $this->BAPSTHPRegulerModel->GetFilterAjaxCount($filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->BAPSTHPRegulerModel->GetSearchAjax($_POST['start'], $_POST['length'], $search, $order, $dir, $filtercond);

            $totalFiltered = $this->BAPSTHPRegulerModel->GetSearchAjaxCount($search, $filtercond);
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
				...
				if($bolehedit)
					$tools .= "<a class='btn btn-xs btn-primary btn-sm' href='".base_url('BAPSTHPReguler/Edit?id=').$post->id."'><i class='glyphicon glyphicon-pencil'></i></a><a class='btn btn-xs btn-info btn-sm' data-href='#' data-toggle='modal' data-record-title='".$post->no_bapsthp."' data-target='#upload-modal' data-record-id='".$post->id."'><i class='glyphicon glyphicon-open-file'></i></a>";

				if($bolehhapus)
					$tools .= "<a class='btn btn-xs btn-danger btn-sm' data-href='".base_url('BAPSTHPReguler/doDelete?id=').$post->id."' data-toggle='modal' data-record-title='".$post->no_bapsthp."' data-target='#confirm-delete'><i class='glyphicon glyphicon-trash'></i></a>";
	...
	public function Add()
	{
		...
		$data = array(
	        'title' => 'INPUT LAPORAN BAP-STHP REGULER',
	        'content-path' => 'PEGADAAN PUSAT / BAP-STHP REGULER / TAMBAH DATA',
	        'content' => $this->load->view('bapsthp-reguler-add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		if($tahun_anggaran == '' or $id_penyerah == '' or $no_kontrak == '' or $nama_barang == '' or $merk == '' or$jumlah_barang == '' or $nilai_barang == '' or $tanggal == ''){
			...
			$this->BAPSTHPRegulerModel->Insert($data);
			$this->session->set_flashdata('info','Data inserted successfully.');
			redirect('BAPSTHPReguler');
	...
	public function Test()
	{
			...
			$bapsthp_reguler = $this->BAPSTHPRegulerModel->Get($id);

			$kodefile_upload = strtotime(NOW);

			if($bapsthp_reguler->nama_file != '' and $bapsthp_reguler->nama_file != 'null' and !is_null($bapsthp_reguler->nama_file) and $bapsthp_reguler->nama_file != '[]')
				$nama_file = json_decode($bapsthp_reguler->nama_file);
			else
				$nama_file = [];
	        ...
	        if(count($nama_file) > 10){
			    ...
			    foreach($_FILES['file']['tmp_name'] as $key => $value) {
			        ...
			        $targetFile =  $target_file = $_SERVER['DOCUMENT_ROOT'].'/upload/bapsthp_reguler/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
				...
				$this->BAPSTHPRegulerModel->Update($id, $data);
	...
	public function RemoveImage()
	{
		...
		if($bapsthp_reguler->nama_file != '' and $bapsthp_reguler->nama_file != 'null' and !is_null($bapsthp_reguler->nama_file) and $bapsthp_reguler->nama_file != '[]')
			$images = json_decode($bapsthp_reguler->nama_file);
		else
			$images = [];

		$nama_file = $images[$urutanfile];
		unlink($_SERVER['DOCUMENT_ROOT'].'/upload/bapsthp_reguler/'.$nama_file);	
	...
	public function Edit()
	{
		...
		$param['bapsthp_reguler'] = $this->BAPSTHPRegulerModel->Get($id);
		...
		$this->load->library('parser');
		$data = array(
	        'title' => 'UBAH LAPORAN BAP-STHP REGULER',
	        'content-path' => 'PEGADAAN PUSAT / BAP-STHP REGULER / UBAH DATA',
	        'content' => $this->load->view('bapsthp-reguler-edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	...
	{	
		...
		foreach($images as $image){
			unlink($_SERVER['DOCUMENT_ROOT'].'/upload/bapsthp_reguler/'.$image);	
		}
		
		$this->BAPSTHPRegulerModel->Delete($id);
	...
	public function GetBAPSTHP()
	{
		...
		$data = $this->BAPSTHPRegulerModel->Get($id_bapsthp);
	...
	public function doExport() {
		...
		$data = array();
		if(empty($this->input->post('search')['value'])){            
			$data = $this->BAPSTHPRegulerModel->ExportAllForAjax($order, $dir, $filtercond);
		}
		else {
			$search = $this->input->post('search')['value'];
			$data =  $this->BAPSTHPRegulerModel->ExportSearchAjax($search, $order, $dir, $filtercond);
		}
		...
		$this->xlsxwriter->writeSheetHeader('BAPSTHP Reguler', $visible_header_columns, array('font-style'=>'bold'));
		...
		foreach($data as $row) {
			...
			$this->xlsxwriter->writeSheetRow('BAPSTHP Reguler', $newRow);
		}

		$uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
		$file = "upload/BASTB App Data BAPSTHP Reguler - $uniq_id.xlsx";
		$this->xlsxwriter->writeToFile($file);

		header('Content-Type: application/json');
		echo json_encode(array('filename' => base_url().'BAPSTHPReguler/doDownload?filename='.$file));
	}
