...
class LaporanProvinsiProvinsi extends CI_Controller {
	...
	public function AjaxGetDataUnit()
	{
		...
		$totalData = $this->LaporanProvinsiModel->GetAllAjaxCountProvUnit($list_nama_barang);
        ...
        {            
            $posts = $this->LaporanProvinsiModel->GetAllForAjaxProvUnit($_POST['start'], $_POST['length'], $order, $dir, $filtercond, $list_nama_barang);
            $totalFiltered = $this->LaporanProvinsiModel->GetFilterAjaxCountProvUnit($filtercond, $list_nama_barang);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->LaporanProvinsiModel->GetSearchAjaxProvUnit($_POST['start'], $_POST['length'], $search, $order, $dir, $filtercond, $list_nama_barang);

            $totalFiltered = $this->LaporanProvinsiModel->GetSearchAjaxCountProvUnit($search, $filtercond, $list_nama_barang);
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                $nestedData['bapsthp'] = number_format($post->total_unit_bapreg, 0);
                $nestedData['persen_bapsthp'] = number_format(($post->total_unit_alokasi == 0 ? 0 : (($post->total_unit_bapreg) / $post->total_unit_alokasi * 100)), 0)."%";
	...
	public function AjaxGetDataNilai()
	{
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                $nestedData['bapsthp_nilai'] = number_format($post->total_nilai_bapreg, 0);
                $nestedData['persen_bapsthp_nilai'] = number_format(($post->total_nilai_alokasi == 0 ? 0 : (($post->total_nilai_bapreg) / $post->total_nilai_alokasi * 100)), 0)."%";
