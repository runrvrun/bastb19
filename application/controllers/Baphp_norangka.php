...
class Baphp_norangka extends CI_Controller {
    public $cols = array(
    ...
    function __construct()
    {
		...
		$this->load->model('Baphp_norangka_model');
    ...
    public function index($id = null)
    {
		...
		if(empty($param['baphp'])){
			show_404();
		}
		$param['cols'] = $this->cols;
		$param['baphp_norangka'] = $this->Baphp_norangka_model->get(null,$id_baphp);
        ...
        $data = array(
            ...
            'content' => $this->load->view('baphp-norangka/index', $param, TRUE),
        );
    ...
    { 
        ...
        $totalData = $this->Baphp_norangka_model->get(null,$id_baphp);
		...
		$posts_all_search =  $this->Baphp_norangka_model->get(null, $id_baphp,null, null, null, null, $filtercond, $search);
        $totalFiltered = count($posts_all_search);
		$posts =  $this->Baphp_norangka_model->get(null, $id_baphp, $start, $length, $order, $dir, $filtercond, $search);
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                if($bolehhapus)
                    $tools = "<a class='btn btn-xs btn-danger btn-sm' data-href='".base_url('Baphp_norangka/destroy?id=').$post->id."' data-toggle='modal' data-record-title='".$post->id."' data-target='#confirm-delete'><i class='glyphicon glyphicon-trash'></i></a>";
    ...
    {
		$baphp_norangka = $this->Baphp_norangka_model->Get($id)[0];
    ...
    public function get_json()
    {
		...
		$data = $this->Baphp_norangka_model->get($id);
    ...
    public function create()
	{
		...
		$param['baphp'] = $this->Baphp_model->get($this->input->get('id_baphp'));
		//Data tidak ditemukan
		if(empty($param['baphp'])){
			show_404();
		}
		$param['id_kontrak_pusat'] = $param['baphp']->id_kontrak_pusat;
		$param['norangka'] = $this->Norangka_model->get(null,array('id_kontrak_pusat'=>$param['baphp']->id_kontrak_pusat));

		$data = array(
			'title' => 'Pilih Nomor Rangka & Nomor Mesin',
            'content-path' => strtoupper('Kontrak Pusat / BAPHP / Nomor Rangka & Nomor Mesin / Pilih Data'),
	        'content' => $this->load->view('baphp-norangka/add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		$baphp_norangka = $this->Baphp_norangka_model->get(null,$this->input->post('id_baphp'));
		$baphp = $this->Baphp_model->get($this->input->post('id_baphp'));
		// var_dump(count($baphp_norangka));exit();
        if(count($baphp_norangka)+$jumlah_input > $baphp->jumlah_barang){
			$this->session->set_flashdata('error','Jumlah nomor rangka tidak dapat melebihi jumlah unit di kontrak.');
			redirect('Baphp_norangka/create?id_baphp='.$id_baphp);
		}

		foreach($id_norangka_array as $key=>$val){
			$data['id_baphp'] = $id_baphp;
			$data['id_norangka'] = $val;
			$this->Baphp_norangka_model->store($data);
		}
        $this->session->set_flashdata('info','Data inserted successfully.');
		redirect('Baphp_norangka/index?id_baphp='.$id_baphp);
	}

	public function destroy()
	{	
        $id = $this->input->get('id');
		$baphp_norangka = $this->Baphp_norangka_model->get($id);
		$id_baphp = $baphp_norangka->id_baphp;

		if($baphp_norangka->nama_file != '' and $baphp_norangka->nama_file != 'null' and !is_null($baphp_norangka->nama_file) and $baphp_norangka->nama_file != '[]')
			$images = json_decode($baphp_norangka->nama_file);
		else
			$images = [];

		foreach($images as $image){
			unlink($this->config->item('doc_root').'/upload/baphp_norangka'.$image);	
		}

		$this->Baphp_norangka_model->destroy($id);
    ...
    public function export()
    {
        ...
        if(empty($this->input->post('search')['value'])){            
            $data = $this->Baphp_norangka_model->ExportAllForAjax($order, $dir, $filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 
            $data =  $this->Baphp_norangka_model->ExportSearchAjax($search, $order, $dir, $filtercond);
        }

        $visible_columns = $this->input->post('visible_columns');
        $visible_header_columns = array();
        $this->xlsxwriter->writeSheetHeader('Baphp_norangka', $visible_header_columns, array('font-style'=>'bold'));
        ...
        foreach($data as $row) {
            ...
            $this->xlsxwriter->writeSheetRow('Baphp_norangka', $newRow);
        }
        
        $uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
        $file = "upload/BASTB App Baphp_norangka - $uniq_id.xlsx";
        $this->xlsxwriter->writeToFile($file);

        header('Content-Type: application/json');
        echo json_encode(array('filename' => base_url().'Baphp_norangka/download?filename='.$file));
    }
    ...
    public function upload_file()
    {
		...
		if($baphp_norangka->nama_file != '' and $baphp_norangka->nama_file != 'null' and !is_null($baphp_norangka->nama_file) and $baphp_norangka->nama_file != '[]')
			$nama_file = json_decode($baphp_norangka->nama_file);
		else
			$nama_file = [];
        ...
        if(count($nama_file) > 10){
		    ...
		    foreach($_FILES['file']['tmp_name'] as $key => $value) {
		        ...
		        $targetFile =  $target_file = $this->config->item('doc_root').'/upload/baphp_norangka'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
			...
			$this->Baphp_norangka_model->update($id, $data);
    ...
    public function remove_file()
    {
		...
		unlink($this->config->item('doc_root').'/upload/baphp_norangka'.$nama_file);	
