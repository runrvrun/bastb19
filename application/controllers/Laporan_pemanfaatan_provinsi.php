...
class Laporan_pemanfaatan_provinsi extends CI_Controller {
	public $cols = array(
    ...
    public function index($id = null)
    {
        ...
        if(!empty($id_bastb_provinsi)){
			$param['bastb_provinsi'] = $this->Bastb_provinsi_model->get($id_bastb_provinsi);
			if(empty($param['bastb_provinsi'])) show_404();
			$param['laporan_pemanfaatan_provinsi_provinsi'] = $this->Laporan_pemanfaatan_provinsi_model->get(null,array('id_bastb_provinsi'=>$id_bastb_provinsi));
		}
        $this->load->library('parser');
	
        $data = array(
            'title' => 'Data Laporan Pemanfaatan Kontrak Pusat',
            'content-path' => 'PENGADAAN PROVINSI / KONTRAK PROVINSI / Laporan Pemanfaatan',
            'content' => $this->load->view('laporan-pemanfaatan-provinsi/index', $param, TRUE),
        );
    ...
    { 
        ...
        if(!empty($id_bastb_provinsi)){		
			$totalData = $this->Laporan_pemanfaatan_provinsi_model->get(null,array('id_bastb_provinsi'=>$id_bastb_provinsi));
		}
        ...
        if(!empty($id_bastb_provinsi)){		
			$posts_all_search =  $this->Laporan_pemanfaatan_provinsi_model->get(null, array('id_bastb_provinsi'=>$id_bastb_provinsi, 'filter'=>$filtercond, 'search'=>$search));
			$posts =  $this->Laporan_pemanfaatan_provinsi_model->get(null, array('id_bastb_provinsi'=>$id_bastb_provinsi, 'start'=>$start, 'length'=>$length, 'order'=>$order, 'dir'=>$dir, 'filter'=>$filtercond, 'search'=>$search));
		}
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
				...
				if($bolehedit)
					$tools .= '<a class="dropdown-item" href="'.base_url('Laporan_pemanfaatan_provinsi/edit?id='.$post->id).'">Ubah Data</a>';
				if($bolehhapus)
					$tools .= '<a class="dropdown-item"  href="#" data-toggle="modal" data-record-title="'.$post->id.'" data-target="#confirm-delete" data-href="'.base_url('Laporan_pemanfaatan_provinsi/destroy/').$post->id.'">Hapus Data</a>';
				$tools .= '</div>
				</div>';
    ...
    public function show($id)
    {
		...
		$bastb_provinsi = $this->Bastb_provinsi_model->get($alokasi_pusat->id_bastb_provinsi);;
		$param['bastb_provinsi'] = $bastb_provinsi;

		$this->load->library('parser');
		$data = array(
	        'title' => 'Data Laporan Pemanfaatan Kontrak Pusat',
	        'content-path' => 'PENGADAAN PROVINSI / KONTRAK PROVINSI / DATA Laporan Pemanfaatan',
    ...
    public function get_json()
    {
		...
		$data = $this->Laporan_pemanfaatan_provinsi_model->get($id);
    ...
    public function create()
	{
		...
		$data = array(
	        ...
	        'content-path' => 'PENGADAAN PROVINSI / KONTRAK PROVINSI / Data Laporan Pemanfaatan / TAMBAH DATA',
	        'content' => $this->load->view('laporan-pemanfaatan-provinsi/add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		if(!empty($this->input->post('id_bastb_provinsi'))){
			$data['id_bastb_provinsi'] = $this->input->post('id_bastb_provinsi');
		}
		...
		$this->Laporan_pemanfaatan_provinsi_model->store($data);
		$this->session->set_flashdata('info','Data inserted successfully.');
		if(!empty($this->input->post('id_bastb_provinsi'))){
			redirect('Laporan_pemanfaatan_provinsi/index?id_bastb_provinsi='.$this->input->post('id_bastb_provinsi'));
		}		
	}
	...
	public function edit()
	{
		...
		$param['laporan_pemanfaatan_provinsi'] = $this->Laporan_pemanfaatan_provinsi_model->get($id);
		if(!empty($param['laporan_pemanfaatan_provinsi']->id_bastb_provinsi)){
			$param['bastb_provinsi'] = $this->Bastb_provinsi_model->get($param['laporan_pemanfaatan_provinsi']->id_bastb_provinsi);
		}
		...
		$data = array(
	        'title' => 'Data Laporan Pemanfaatan Kontrak Pusat',
	        'content-path' => 'PENGADAAN PROVINSI / KONTRAK PROVINSI / Data Laporan Pemanfaatan / UBAH DATA',
	        'content' => $this->load->view('laporan-pemanfaatan-provinsi/edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		$periode_selesai = DateTime::createFromFormat('d-m-Y', $this->input->post('periode_selesai'))->format('Y-m-d');
		$tanggal_laporan = DateTime::createFromFormat('d-m-Y', $this->input->post('tanggal_laporan'))->format('Y-m-d');
		$laporan_pemanfaatan_provinsi = $this->Laporan_pemanfaatan_provinsi_model->get($id);
		$data = array(
		...
		$this->Laporan_pemanfaatan_provinsi_model->update($id, $data);
		$this->session->set_flashdata('info','Data updated successfully.');
		if(!empty($laporan_pemanfaatan_provinsi->id_bastb_provinsi)){
			redirect('Laporan_pemanfaatan_provinsi/index?id_bastb_provinsi='.$laporan_pemanfaatan_provinsi->id_bastb_provinsi);
		}
	}
	...
	{	
		$laporan_pemanfaatan_provinsi = $this->Laporan_pemanfaatan_provinsi_model->get($id);
		$id_bastb_provinsi = $laporan_pemanfaatan_provinsi->id_bastb_provinsi;

		if($laporan_pemanfaatan_provinsi->nama_file != '' and $laporan_pemanfaatan_provinsi->nama_file != 'null' and !is_null($laporan_pemanfaatan_provinsi->nama_file) and $laporan_pemanfaatan_provinsi->nama_file != '[]')
			$images = json_decode($laporan_pemanfaatan_provinsi->nama_file);
		else
			$images = [];

		foreach($images as $image){
			unlink($this->config->item('doc_root').'/upload/laporan_pemanfaatan_provinsi/'.$image);	
		}

		$this->Laporan_pemanfaatan_provinsi_model->destroy($id);
		$this->session->set_flashdata('info','Data deleted successfully.');
		redirect('Laporan_pemanfaatan_provinsi/index?id_bastb_provinsi='.$id_bastb_provinsi);
    ...
    public function export()
    {
        ...
        if(empty($this->input->post('search')['value'])){            
            $data = $this->Laporan_pemanfaatan_provinsi_model->ExportAllForAjax($order, $dir, $filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 
            $data =  $this->Laporan_pemanfaatan_provinsi_model->ExportSearchAjax($search, $order, $dir, $filtercond);
        }
        ...
        echo json_encode(array('filename' => base_url().'Laporan_pemanfaatan_provinsi/download?filename='.$file));
    }
    ...
    public function upload_file()
    {
		...
		if($laporan_pemanfaatan_provinsi->nama_file != '' and $laporan_pemanfaatan_provinsi->nama_file != 'null' and !is_null($laporan_pemanfaatan_provinsi->nama_file) and $laporan_pemanfaatan_provinsi->nama_file != '[]')
			$nama_file = json_decode($laporan_pemanfaatan_provinsi->nama_file);
		else
			$nama_file = [];
        ...
        if(count($nama_file) > 10){
		    ...
		    foreach($_FILES['file']['tmp_name'] as $key => $value) {
		        ...
		        $targetFile =  $target_file = $this->config->item('doc_root').'/upload/laporan_pemanfaatan_provinsi/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
    ...
    public function remove_file()
    {
		...
		unlink($this->config->item('doc_root').'/upload/laporan_pemanfaatan_provinsi/'.$nama_file);	
