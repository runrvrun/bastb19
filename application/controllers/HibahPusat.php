...
class HibahPusat extends CI_Controller {
	...
	public function index()
	{
		...
		$data = array(
	        ...
	        'content' => $this->load->view('hibah-pusat-index', $param, TRUE),
		);
	...
	public function AjaxGetAllData()
	{
		...
		$totalData = $this->HibahPusatModel->GetAllAjaxCount();
        ...
        {            
            $posts = $this->HibahPusatModel->GetAllForAjax($_POST['start'], $_POST['length'], $order, $dir, $filtercond);
            $totalFiltered = $this->HibahPusatModel->GetFilterAjaxCount($filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->HibahPusatModel->GetSearchAjax($_POST['start'], $_POST['length'], $search, $order, $dir, $filtercond);

            $totalFiltered = $this->HibahPusatModel->GetSearchAjaxCount($search, $filtercond);
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                if($post->tanggal_naskah_hibah and $post->tanggal_naskah_hibah != '0000-00-00')
                ...
                if($post->tanggal_bast_bmn and $post->tanggal_bast_bmn != '0000-00-00')
                ...
                $tools = "";
                $tools .= "<a class='btn btn-xs btn-success btn-sm' href='".base_url('HibahPusat/LihatDokumen?id=').$post->id."'><i class='glyphicon glyphicon-zoom-in'></i></a>";

                if($bolehedit)
                	$tools .= "<a class='btn btn-xs btn-primary btn-sm' href='".base_url('HibahPusat/Edit?id=').$post->id."'><i class='glyphicon glyphicon-pencil'></i></a>";
                ...
                if($bolehhapus)
                	$tools .= "<a class='btn btn-xs btn-danger btn-sm' data-href='".base_url('HibahPusat/doDelete?id=').$post->id."' data-toggle='modal' data-record-title='".$post->no_naskah_hibah."' data-target='#confirm-delete'><i class='glyphicon glyphicon-trash'></i></a>";
	...
	{
		$this->load->library('parser');
		$setting = $this->SettingHibahPusatModel->GetSettingUser($this->session->userdata('logged_in')->id_pengguna);

		if(!$setting){
			$this->SettingHibahPusatModel->InsertDefaultData();
			$setting = $this->SettingHibahPusatModel->GetSettingUser($this->session->userdata('logged_in')->id_pengguna);
		}
		...
		$data = array(
	        'title' => 'SETTING UMUM',
	        'content-path' => 'PEGADAAN PUSAT / HIBAH / SETTING UMUM',
	        'content' => $this->load->view('setting-hibah-pusat', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		if($nama_penyerah == '' or $nip_penyerah == '' or $pangkat_penyerah == '' or $jabatan_penyerah == '' or$alamat_dinas_penyerah == '' or $instansi_penerima == '' or $nama_penerima == '' or $nip_penerima == '' or $pangkat_penerima == '' or $jabatan_penerima == '' or $alamat_dinas_penerima == ''){
			$this->session->set_flashdata('error','All fields must be filled.');
     		redirect('HibahPusat/SettingUmum');
		}
		else{
			...
			$this->SettingHibahPusatModel->SaveData($id, $data);
   ...
   //   			redirect('HibahPusat/Add');
			...
			$no_naskah_hibah_terpakai = $this->HibahPusatModel->CheckNoNaskahHibah($no_naskah_hibah);
			$no_bast_bmn_terpakai = $this->HibahPusatModel->CheckNoBASTBMN($no_bast_bmn);
			$no_surat_pernyataan_terpakai = $this->HibahPusatModel->CheckNoSuratPernyataan($no_surat_pernyataan);
   ...
   //   			redirect('HibahPusat/Add');
			...
			$this->HibahPusatModel->Insert($data);
		...
		// if($no_naskah_hibah != ''){
	        ...
	        $baris = $baris + 1;
	 		...
	 		$pdf->Cell(10, 0.5, 'a.n Menteri Pertanian', 0 , 1, 'C');
	        ...
	        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/naskah_hibah/'.$this->clean($no_naskah_hibah).'_'.$idinserted.'.pdf', 'F');
	        // $pdf->Output();
	        // exit(1);
			...
			$pdf->Cell(13, 0.5, 'a.n Menteri Pertanian', 0 , 1, 'C');
	        ...
	        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_naskah_hibah/lamp_'.$this->clean($no_naskah_hibah).'_'.$idinserted.'.pdf', 'F');
	        // $pdf->Output();
	        // exit(1);
		// }
		...
		// if($no_bast_bmn != ''){
	        ...
	        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/bast_bmn/'.$this->clean($no_bast_bmn).'_'.$idinserted.'.pdf', 'F');
	        // $pdf->Output();
	        // exit(1);
	        ...
	        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_bast_bmn/lamp_'.$this->clean($no_bast_bmn).'_'.$idinserted.'.pdf', 'F');
			// $pdf->Output();
			// exit(1);
	    // }
        // ************************************************************************************************************************
	    ...
	    // if($no_surat_pernyataan != '' and $tanggal_surat_pernyataan != ''){
	        ...
	        if($tanggal_surat_pernyataan != ''){
				$hari = 'Senin';
				$bulan = 'Januari';
				$mo = date('n', strtotime($tanggal_surat_pernyataan));

				$day = date('D', strtotime($tanggal_surat_pernyataan));
				if($day == 'Mon')
					$hari = 'Senin';
				if($day == 'Tue')
					$hari = 'Selasa';
				...
				$tanggal = ucwords($this->terbilang(date('j', strtotime($tanggal_surat_pernyataan))));
				$tahun = ucwords($this->terbilang(date('Y', strtotime($tanggal_surat_pernyataan))));
			}
			else{
				$hari = '...............';
				$tanggal = '......';
	        ...
	        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/surat_pernyataan/'.$this->clean($no_surat_pernyataan).'_'.$idinserted.'.pdf', 'F');
	 	// 	$pdf->Output();
			// exit(1);
	        ...
	        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_surat_pernyataan/lamp_'.$this->clean($no_surat_pernyataan).'_'.$idinserted.'.pdf', 'F');
			// $pdf->Output();
			// exit(1);
	    // }

        $this->session->set_flashdata('info','Data inserted successfully.');
		redirect('HibahPusat/LihatDokumen?id='.$idinserted);
		// exit(1);
	...
	public function LihatDokumen()
	{
		...
		$param['no_surat_pernyataan'] = $this->clean($hibah_pusat->no_surat_pernyataan);

		$this->load->library('parser');
		$data = array(
	        'title' => 'LIHAT DOKUMEN',
	        'content-path' => 'PEGADAAN PUSAT / HIBAH / LIHAT DOKUMEN',
	        'content' => $this->load->view('hibah-pusat-lihat-dokumen', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	...
	public function Test()
	{
			...
			$id = $this->input->post('id_hibah_pusat');

			$hibah_pusat = $this->HibahPusatModel->Get($id);

			$kodefile_upload = strtotime(NOW);

			if($hibah_pusat->nama_file != '' and $hibah_pusat->nama_file != 'null' and !is_null($hibah_pusat->nama_file) and $hibah_pusat->nama_file != '[]')
				$nama_file = json_decode($hibah_pusat->nama_file);
			else
				$nama_file = [];
	        ...
	        if(count($nama_file) > 10){
			    ...
			    foreach($_FILES['file']['tmp_name'] as $key => $value) {
			        ...
			        $targetFile =  $target_file = $_SERVER['DOCUMENT_ROOT'].'/upload/hibah_pusat/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
	...
	public function RemoveImage()
	{
		...
		$id = $this->input->get('id_hibah_pusat');
		$urutanfile = $this->input->get('urutanfile');

		$hibah_pusat = $this->HibahPusatModel->Get($id);

		if($hibah_pusat->nama_file != '' and $hibah_pusat->nama_file != 'null' and !is_null($hibah_pusat->nama_file) and $hibah_pusat->nama_file != '[]')
			$images = json_decode($hibah_pusat->nama_file);
		else
			$images = [];

		$nama_file = $images[$urutanfile];
		unlink($_SERVER['DOCUMENT_ROOT'].'/upload/hibah_pusat/'.$nama_file);	
	...
	public function BuatDokumen()
	{
		...
		$data = array(
	        'title' => 'BUAT DOKUMEN',
	        'content-path' => 'PEGADAAN PUSAT / HIBAH / BUAT DOKUMEN',
	        'content' => $this->load->view('hibah-pusat-buat-dokumen', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	...
	public function Edit()
	{
		...
		$param['hibah_pusat'] = $this->HibahPusatModel->Get($id);
		...
		$data = array(
	        'title' => 'UBAH HIBAH',
	        'content-path' => 'PEGADAAN PUSAT / HIBAH / UBAH DATA',
	        'content' => $this->load->view('hibah-pusat-edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	...
	{	
		...
		if($nama_penyerah == '' or $nip_penyerah == '' or $pangkat_penyerah == '' or $jabatan_penyerah == '' or $alamat_dinas_penyerah == '' or $titik_serah == '' or $nama_wilayah == '' or $instansi_penerima == '' or $nama_penerima == '' or $nip_penerima == ''  or $pangkat_penerima == ''  or $jabatan_penerima == ''  or $alamat_dinas_penerima == ''){
			$this->session->set_flashdata('error','All fields must be filled.');
     		redirect('HibahPusat/Edit?id='.$id);
		}
		else{
   ...
   //   			redirect('HibahPusat/Edit?id='.$id);
			...
			if($hibah_pusat->no_naskah_hibah != $no_naskah_hibah or strtotime($hibah_pusat->tanggal_naskah_hibah) != strtotime($tanggal_naskah_hibah)){
				$ganti_naskah_hibah = 1;
			}
			if($hibah_pusat->no_bast_bmn != $no_bast_bmn or strtotime($hibah_pusat->tanggal_bast_bmn) != strtotime($tanggal_bast_bmn)){
				$ganti_bast_bmn = 1;
			}
			if($hibah_pusat->no_surat_pernyataan != $no_surat_pernyataan or strtotime($hibah_pusat->tanggal_surat_pernyataan) != strtotime($tanggal_surat_pernyataan)){
				$ganti_surat_pernyataan = 1;
			}
			
			if($ganti_naskah_hibah == 1 and $hibah_pusat->no_naskah_hibah != $no_naskah_hibah){
				$no_naskah_hibah_terpakai = $this->HibahPusatModel->CheckNoNaskahHibah($no_naskah_hibah);
	   ...
	   //   			redirect('HibahPusat/Edit?id='.$id);
				// }
				//hapus file lama
				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/naskah_hibah/'.$this->clean($hibah_pusat->no_naskah_hibah).'_'.$id.'.pdf'))
					unlink($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/naskah_hibah/'.$this->clean($hibah_pusat->no_naskah_hibah).'_'.$id.'.pdf');
				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_naskah_hibah/lamp_'.$this->clean($hibah_pusat->no_naskah_hibah).'_'.$id.'.pdf'))
					unlink($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_naskah_hibah/lamp_'.$this->clean($hibah_pusat->no_naskah_hibah).'_'.$id.'.pdf');
			}

			if($ganti_bast_bmn == 1 and $hibah_pusat->no_bast_bmn != $no_bast_bmn){
				$no_bast_bmn_terpakai = $this->HibahPusatModel->CheckNoBASTBMN($no_bast_bmn);
				// if($no_bast_bmn_terpakai and $no_bast_bmn != ''){
				// 	$this->session->set_flashdata('error','No BAST BMN sudah digunakan. Silahkan periksa kembali data anda.');
	   //   			redirect('HibahPusat/Edit?id='.$id);
				// }
				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/bast_bmn/'.$this->clean($hibah_pusat->no_bast_bmn).'_'.$id.'.pdf'))	
					unlink($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/bast_bmn/'.$this->clean($hibah_pusat->no_bast_bmn).'_'.$id.'.pdf');
				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_bast_bmn/lamp_'.$this->clean($hibah_pusat->no_bast_bmn).'_'.$id.'.pdf'))
					unlink($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_bast_bmn/lamp_'.$this->clean($hibah_pusat->no_bast_bmn).'_'.$id.'.pdf');
			}

			if($ganti_surat_pernyataan == 1 and $hibah_pusat->no_surat_pernyataan != $no_surat_pernyataan){
				// $no_surat_pernyataan_terpakai = $this->HibahPusatModel->CheckNoSuratPernyataan($no_surat_pernyataan);
				// if($no_surat_pernyataan_terpakai and $no_surat_pernyataan != ''){
				// 	$this->session->set_flashdata('error','No Surat Pernyataan sudah digunakan. Silahkan periksa kembali data anda.');
	   //   			redirect('HibahPusat/Edit?id='.$id);
				// }
				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/surat_pernyataan/'.$this->clean($hibah_pusat->no_surat_pernyataan).'_'.$id.'.pdf'))
					unlink($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/surat_pernyataan/'.$this->clean($hibah_pusat->no_surat_pernyataan).'_'.$id.'.pdf');

				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_surat_pernyataan/lamp_'.$this->clean($hibah_pusat->no_surat_pernyataan).'_'.$id.'.pdf'))
					unlink($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_surat_pernyataan/lamp_'.$this->clean($hibah_pusat->no_surat_pernyataan).'_'.$id.'.pdf');
			}
			...
			// if($no_naskah_hibah != ''){
		        ...
		        $baris = $baris + 1;
		 		...
		 		$pdf->Cell(9, 0.5, 'NIP. '.$nip_penerima, 0 , 0, 'C');
		 		$pdf->Cell(10, 0.5, 'NIP. '.$nip_penyerah, 0 , 1, 'C');
		        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/naskah_hibah/'.$this->clean($no_naskah_hibah).'_'.$id.'.pdf', 'F');
		        // $pdf->Output();
		        // exit(1);
		        ...
		        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_naskah_hibah/lamp_'.$this->clean($no_naskah_hibah).'_'.$id.'.pdf', 'F');
		        // $pdf->Output();
		        // exit(1);
			// }
			...
			// if($no_bast_bmn != ''){
		        ...
		        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/bast_bmn/'.$this->clean($no_bast_bmn).'_'.$id.'.pdf', 'F');
		        // $pdf->Output();
		        // exit(1);
		        ...
		        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_bast_bmn/lamp_'.$this->clean($no_bast_bmn).'_'.$id.'.pdf', 'F');
				// $pdf->Output();
				// exit(1);
		    // }
		    ...
		    // if($no_surat_pernyataan != '' and $tanggal_surat_pernyataan != ''){
		        ...
		        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/surat_pernyataan/'.$this->clean($no_surat_pernyataan).'_'.$id.'.pdf', 'F');
		  //       $pdf->Output();
				// exit(1);
		        ...
		        $pdf->Output($_SERVER['DOCUMENT_ROOT'].'/pdf_hibah/hibah_pusat/lamp_surat_pernyataan/lamp_'.$this->clean($no_surat_pernyataan).'_'.$id.'.pdf', 'F');
				// $pdf->Output();
				// exit(1);
		    // }
			...
			$this->session->set_flashdata('info','Data updated successfully.');
			redirect('HibahPusat/LihatDokumen?id='.$id);
			// redirect('HibahPusat');
	...
	{
		$id_hibah_pusat = $this->input->get('id_hibah_pusat');

		$data = $this->HibahPusatModel->Get($id_hibah_pusat);
