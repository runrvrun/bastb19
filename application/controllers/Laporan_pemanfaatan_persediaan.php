...
class Laporan_pemanfaatan_persediaan extends CI_Controller {
	public $cols = array(
    ...
    function __construct()
    {
		...
		$this->load->model('Laporan_pemanfaatan_persediaan_model');
    ...
    public function index($id = null)
    {
		...
		if(!empty($id_bastb_persediaan)){
			$param['bastb_persediaan'] = $this->Bastb_persediaan_model->get($id_bastb_persediaan);
			if(empty($param['bastb_persediaan'])) show_404();
			$param['Laporan_pemanfaatan_persediaan'] = $this->Laporan_pemanfaatan_persediaan_model->get(null,array('id_bastb_persediaan'=>$id_bastb_persediaan));
        }
        ...
        $data = array(
            ...
            'content' => $this->load->view('laporan-pemanfaatan-persediaan/index', $param, TRUE),
        );
    ...
    { 
		...
		if(!empty($id_bastb_persediaan)){		
			$totalData = $this->Laporan_pemanfaatan_persediaan_model->get(null,array('id_bastb_persediaan'=>$id_bastb_persediaan));
		}
        ...
        if(!empty($id_bastb_persediaan)){		
			$posts_all_search =  $this->Laporan_pemanfaatan_persediaan_model->get(null, array('id_bastb_persediaan'=>$id_bastb_persediaan, 'filter'=>$filtercond, 'search'=>$search));
			$posts =  $this->Laporan_pemanfaatan_persediaan_model->get(null, array('id_bastb_persediaan'=>$id_bastb_persediaan, 'start'=>$start, 'length'=>$length, 'order'=>$order, 'dir'=>$dir, 'filter'=>$filtercond, 'search'=>$search));
		}
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
				...
				if($bolehedit)
					$tools .= '<a class="dropdown-item" href="'.base_url('Laporan_pemanfaatan_persediaan/edit?id='.$post->id).'">Ubah Data</a>';
				if($bolehhapus)
					$tools .= '<a class="dropdown-item"  href="#" data-toggle="modal" data-record-title="'.$post->id.'" data-target="#confirm-delete" data-href="'.base_url('Laporan_pemanfaatan_persediaan/destroy/').$post->id.'">Hapus Data</a>';
				$tools .= '</div>
				</div>';
    ...
    {
		$bastb_persediaan = $this->Bastb_persediaan_model->get($bastb_persediaan->id);;
		$param['bastb_persediaan'] = $bastb_persediaan;
    ...
    public function get_json()
    {
		...
		$data = $this->Laporan_pemanfaatan_persediaan_model->get($id);
    ...
    public function create()
	{
		...
		$data = array(
	        ...
	        'content' => $this->load->view('laporan-pemanfaatan-persediaan/add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		if(!empty($this->input->post('id_bastb_persediaan'))){
			$data['id_bastb_persediaan'] = $this->input->post('id_bastb_persediaan');
		}
		...
		$this->Laporan_pemanfaatan_persediaan_model->store($data);
		$this->session->set_flashdata('info','Data inserted successfully.');
		redirect('Laporan_pemanfaatan_persediaan/index?id_bastb_persediaan='.$this->input->post('id_bastb_persediaan'));
	}
	...
	public function edit()
	{
		...
		$param['laporan_pemanfaatan_persediaan'] = $this->Laporan_pemanfaatan_persediaan_model->get($id);
		if(!empty($param['laporan_pemanfaatan_persediaan']->id_bastb_persediaan)){
			$param['bastb_persediaan'] = $this->Bastb_persediaan_model->get($param['laporan_pemanfaatan_persediaan']->id_bastb_persediaan);
		}
		...
		$data = array(
	        ...
	        'content' => $this->load->view('laporan-pemanfaatan-persediaan/edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		$periode_selesai = DateTime::createFromFormat('d-m-Y', $this->input->post('periode_selesai'))->format('Y-m-d');
		$tanggal_laporan = DateTime::createFromFormat('d-m-Y', $this->input->post('tanggal_laporan'))->format('Y-m-d');
		$laporan_pemanfaatan_persediaan = $this->Laporan_pemanfaatan_persediaan_model->get($id);
		$data = array(
		...
		$this->Laporan_pemanfaatan_persediaan_model->update($id, $data);
		$this->session->set_flashdata('info','Data updated successfully.');
		redirect('Laporan_pemanfaatan_persediaan/index?id_bastb_persediaan='.$laporan_pemanfaatan_persediaan->id_bastb_persediaan);
	}

	public function destroy($id)
	{	
		$laporan_pemanfaatan_persediaan = $this->Laporan_pemanfaatan_persediaan_model->get($id);
		$id_bastb_persediaan = $laporan_pemanfaatan_persediaan->id_bastb_persediaan;

		if($laporan_pemanfaatan_persediaan->nama_file != '' and $laporan_pemanfaatan_persediaan->nama_file != 'null' and !is_null($laporan_pemanfaatan_persediaan->nama_file) and $laporan_pemanfaatan_persediaan->nama_file != '[]')
			$images = json_decode($laporan_pemanfaatan_persediaan->nama_file);
		else
			$images = [];

		foreach($images as $image){
			unlink($this->config->item('doc_root').'/upload/laporan_pemanfaatan_persediaan/'.$image);	
		}

		$this->Laporan_pemanfaatan_persediaan_model->destroy($id);
		$this->session->set_flashdata('info','Data deleted successfully.');
		redirect('Laporan_pemanfaatan_persediaan/index?id_bastb_persediaan='.$id_bastb_persediaan);
    ...
    public function export()
    {
        ...
        if(empty($this->input->post('search')['value'])){            
            $data = $this->Laporan_pemanfaatan_persediaan_model->ExportAllForAjax($order, $dir, $filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 
            $data =  $this->Laporan_pemanfaatan_persediaan_model->ExportSearchAjax($search, $order, $dir, $filtercond);
        }
        ...
        echo json_encode(array('filename' => base_url().'Laporan_pemanfaatan_persediaan/download?filename='.$file));
    }
    ...
    public function upload_file()
    {
		...
		if($laporan_pemanfaatan_persediaan->nama_file != '' and $laporan_pemanfaatan_persediaan->nama_file != 'null' and !is_null($laporan_pemanfaatan_persediaan->nama_file) and $laporan_pemanfaatan_persediaan->nama_file != '[]')
			$nama_file = json_decode($laporan_pemanfaatan_persediaan->nama_file);
		else
			$nama_file = [];
        ...
        if(count($nama_file) > 10){
		    ...
		    foreach($_FILES['file']['tmp_name'] as $key => $value) {
		        ...
		        $targetFile =  $target_file = $this->config->item('doc_root').'/upload/laporan_pemanfaatan_persediaan/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
    ...
    public function remove_file()
    {
		...
		unlink($this->config->item('doc_root').'/upload/laporan_pemanfaatan_persediaan/'.$nama_file);	
