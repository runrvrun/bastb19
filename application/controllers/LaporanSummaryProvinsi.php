...
class LaporanSummaryProvinsi extends CI_Controller {
	...
	public function index()
	{
		...
		$param['penyedia'] = $this->PenyediaProvinsiModel->GetAll();

		$data = array(
	        'title' => 'SUMMARY',
	        'content-path' => 'PENGADAAN TP PROVINSI / LAPORAN / SUMMARY',
	        'content' => $this->load->view('laporan-summary-provinsi', $param, TRUE),
		);
	...
	public function GetReportData()
	{
		...
		$count_bapsthp_reg = $this->LaporanProvinsiModel->GetTotalBAPSTHP($tahun_anggaran, $list_id_provinsi, $list_id_kabupaten, $list_id_penyedia);
		$count_bapsthp = $count_bapsthp_reg;
		$count_bastb = $this->LaporanProvinsiModel->GetTotalBASTB($tahun_anggaran, $list_id_provinsi, $list_id_kabupaten, $list_id_penyedia);
		...
		$sum_unit_bapsthp_reg = $this->LaporanProvinsiModel->GetTotalUnitBAPSTHP($tahun_anggaran, $list_id_provinsi, $list_id_kabupaten, $list_id_penyedia);
		$sum_unit_bapsthp = $sum_unit_bapsthp_reg;
		$sum_unit_bastb = $this->LaporanProvinsiModel->GetTotalUnitBASTB($tahun_anggaran, $list_id_provinsi, $list_id_kabupaten, $list_id_penyedia);
		...
		$sum_nilai_bapsthp_reg = $this->LaporanProvinsiModel->GetTotalNilaiBAPSTHP($tahun_anggaran, $list_id_provinsi, $list_id_kabupaten, $list_id_penyedia);
		$sum_nilai_bapsthp = $sum_nilai_bapsthp_reg;
		$sum_nilai_bastb = $this->LaporanProvinsiModel->GetTotalNilaiBASTB($tahun_anggaran, $list_id_provinsi, $list_id_kabupaten, $list_id_penyedia);
	...
	public function ExportPDF()
	{
		...
		if($list_id_penyedia == ""){
			...
			for($i=0; $i<count($list_id_penyedia); $i++){
				$temp = $this->PenyediaProvinsiModel->Get($list_id_penyedia[$i]);
				if($i==0)
					$peny .= $temp->nama_penyedia_provinsi;
				else
					$peny .= ', '.$temp->nama_penyedia_provinsi;
			}
        ...
        $pdf->SetXY(1, 0);
        $pdf->Cell(0, 2,'LAPORAN SUMMARY TP PROVINSI', 0, 1, 'C');
		...
		$pdf->Output($_SERVER['DOCUMENT_ROOT'].'/upload/report/LaporanSummaryProvinsi.pdf', 'F');
