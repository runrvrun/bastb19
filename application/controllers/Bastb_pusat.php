...
class Bastb_pusat extends CI_Controller { 
	public $cols = array(
		...
		array("column" => "nama_barang", "caption" => "Nama Barang", "dbcolumn" => "tb_bastb_pusat.nama_barang"),
		array("column" => "merk", "caption" => "Merk Barang", "dbcolumn" => "tb_bastb_pusat.merk"),
    ...
    public function index($id = null)
    {
        ...
        if(!empty($id_alokasi)){
			$param['Bastb_pusat'] = $this->Bastb_pusat_model->get(null,$id_alokasi);
        ...
        if(!empty($id_alokasi)){
			$param['total_unit'] = $this->Bastb_pusat_model->total_unit($id_alokasi);
			$param['total_nilai'] = $this->Bastb_pusat_model->total_nilai($id_alokasi);
		}else{
			$param['total_unit'] = $this->Bastb_pusat_model->total_unit_regcad();
			$param['total_nilai'] = $this->Bastb_pusat_model->total_nilai_regcad();
		}
		$param['total_all_unit'] = $this->Bastb_pusat_model->total_all_unit();
		$param['total_all_nilai'] = $this->Bastb_pusat_model->total_all_nilai();
        $data = array(
			'title' => 'Data BASTB Kontrak Pusat',
            'content-path' => 'PENGADAAN PUSAT / BASTB',
            'content' => $this->load->view('bastb-pusat/index', $param, TRUE),
		);
    ...
    { 
		...
		if(!empty($id_alokasi)){
			...
			$totalData = $this->Bastb_pusat_model->get_regcad_count();
		}
		...
		if(!empty($id_alokasi)){
			...
			$totalFiltered =  $this->Bastb_pusat_model->get_regcad_count(null, $id_alokasi, null, null, null, null, $filtercond, $search);
			$posts =  $this->Bastb_pusat_model->get_regcad(null, $id_alokasi, $start, $length, $order, $dir, $filtercond, $search);
		}
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
				...
				if($post->source == 'reg'){
					...
					<a class="dropdown-item" href="'.base_url('Bastb_pusat_norangka?id_bastb_pusat=').$post->id.'">No. Rangka & No. Mesin</a>';
					if($this->session->userdata('logged_in')->role_pengguna != 'ADMIN KHUSUS') $tools .= '<a class="dropdown-item" href="'.base_url('Laporan_pemanfaatan?id_bastb_pusat=').$post->id.'">Pemanfaatan</a>';
					// $tools .= '<a class="dropdown-item" href="'.base_url('Laporan_pemanfaatan?id_bastb_pusat=').$post->id.'">Pemanfaatan</a>';
				}else{
					$tools .= '
					<a class="dropdown-item" href="#" onclick="LoadDataCad('.$post->id.')">Lihat Data</a> 
					<a class="dropdown-item" href="#" data-href="#" data-toggle="modal" data-record-title="'.$post->id.'" data-target="#upload-modal-cad" data-record-id="'.$post->id.'">Upload Dokumen</a>
					<a class="dropdown-item" href="#" data-href="#" data-toggle="modal" data-record-title="'.$post->id.'" data-target="#upload-modalfoto-cad" data-record-id="'.$post->id.'">Upload Foto Penyerahan</a>
					<a class="dropdown-item" href="'.base_url('Bastb_persediaan_norangka?id_bastb_persediaan=').$post->id.'">No. Rangka & No. Mesin</a>';
					if($this->session->userdata('logged_in')->role_pengguna != 'ADMIN KHUSUS') $tools .= '<a class="dropdown-item" href="'.base_url('Laporan_pemanfaatan_persediaan?id_bastb_persediaan=').$post->id.'">Pemanfaatan</a>';
					// $tools .= '<a class="dropdown-item" href="'.base_url('Laporan_pemanfaatan_persediaan?id_bastb_persediaan=').$post->id.'">Pemanfaatan</a>';
				}
				if($bolehedit)
					if(isset($post->source) && ($post->source == 'cad')){
						$tools .= '<a class="dropdown-item" href="'.base_url('Bastb_persediaan/edit?id='.$post->id).'">Ubah Data</a>';
					}else{
						$tools .= '<a class="dropdown-item" href="'.base_url('Bastb_pusat/edit?id='.$post->id).'">Ubah Data</a>';
					}
				if($bolehhapus)
					$tools .= '<a class="dropdown-item" href="#" data-toggle="modal" data-record-title="'.$post->id.'" data-target="#confirm-delete" data-href="'.base_url('Bastb_pusat/destroy/').$post->id.'">Hapus Data</a>';
				$tools .= '</div>
				</div>';
    ...
    public function get_json()
    {
		...
		$data = $this->Bastb_pusat_model->get($id);
    ...
    public function create()
	{
		...
		$data = array(
	        'title' => 'Input BASTB Kontrak Pusat',
	        'content-path' => 'PENGADAAN PUSAT / DATA BASTB / TAMBAH DATA',
	        'content' => $this->load->view('bastb-pusat/add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		$total_unit = $this->Bastb_pusat_model->total_unit($alokasi_pusat->id);
		if($total_unit+$jumlah_barang > $alokasi_pusat->jumlah_barang_rev){
			$this->session->set_flashdata('error','Jumlah atau nilai tidak dapat melebihi nilai alokasi.');
            redirect('Bastb_pusat/create?id_alokasi='.$alokasi_pusat->id);
		}
		...
		$this->Bastb_pusat_model->store($data);
		$this->session->set_flashdata('info','Data inserted successfully.');
		redirect('Bastb_pusat/index?id_alokasi='.$this->input->post('id_alokasi'));
	}
	...
	public function edit()
	{
		...
		$param['bastb_pusat'] = $this->Bastb_pusat_model->get($id);
		$param['alokasi_pusat'] = $this->Alokasi_pusat_model->get($param['bastb_pusat']->id_alokasi_pusat);
		// var_dump($param['alokasi_pusat']);exit();
		$param['kontrak_pusat'] = $this->Kontrak_pusat_model->get($param['alokasi_pusat']->id_kontrak_pusat);
		$param['penyedia_pusat'] = $this->PenyediaPusatModel->GetAll();
		...
		$data = array(
	        'title' => 'Edit Data BASTB Kontrak Pusat',
	        'content-path' => 'PENGADAAN PUSAT / DATA BASTB / UBAH DATA',
	        'content' => $this->load->view('bastb-pusat/edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		$id = $this->input->post('id');
		$bastb_pusat = $this->Bastb_pusat_model->get($id);
		...
		//cek unit limit 
		$jumlah_barang = $this->input->post('jumlah_barang');
		$alokasi_pusat = $this->Alokasi_pusat_model->get($bastb_pusat->id_alokasi_pusat);
		$total_unit = $this->Bastb_pusat_model->total_unit($alokasi_pusat->id);
		// var_dump($alokasi_pusat->jumlah_barang_rev);var_dump($total_unit);var_dump($jumlah_barang);var_dump($bastb_pusat->jumlah_barang);exit();
		if($total_unit+$jumlah_barang-$bastb_pusat->jumlah_barang > $alokasi_pusat->jumlah_barang_rev){
			$this->session->set_flashdata('error','Jumlah atau nilai tidak dapat melebihi nilai alokasi.');
            redirect('Bastb_pusat/edit?id='.$id);
		}
		...
		// var_dump($data);exit();
		$this->Bastb_pusat_model->update($id, $data);
		$this->session->set_flashdata('info','Data updated successfully.');
		redirect('Bastb_pusat/index?id_alokasi='.$bastb_pusat->id_alokasi_pusat);
	}

	public function destroy($id)
	{	
		$bastb_pusat = $this->Bastb_pusat_model->get($id);
		$id_alokasi_pusat = $bastb_pusat->id_alokasi_pusat;

		if($bastb_pusat->nama_file != '' and $bastb_pusat->nama_file != 'null' and !is_null($bastb_pusat->nama_file) and $bastb_pusat->nama_file != '[]')
			$images = json_decode($bastb_pusat->nama_file);
		else
			$images = [];

		foreach($images as $image){
			unlink($this->config->item('doc_root').'/upload/bastb_pusat/'.$image);	
		}

		$this->Bastb_pusat_model->destroy($id);
		$this->session->set_flashdata('info','Data deleted successfully.');
		redirect('Bastb_pusat/index?id_alokasi='.$id_alokasi_pusat);
    ...
    public function export()
    {
        ...
        if(empty($this->input->post('search')['value'])){            
            $data = $this->Bastb_pusat_model->get_regcad(null,null,null,null,$order, $dir, $filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 
            $data =  $this->Bastb_pusat_model->get_regcad(null,null,null,null, $order, $dir, $filtercond,$search);
        }
        ...
        $this->xlsxwriter->writeSheetHeader('Rekap BASTB Pusat', $visible_header_columns, array('font-style'=>'bold'));
        ...
        foreach($data as $row) {
            ...
            $this->xlsxwriter->writeSheetRow('Rekap BASTB Pusat', $newRow);
        }
        
        $uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
        $file = "tmp_export/Rekap BASTB Pusat - $uniq_id.xlsx";
        $this->xlsxwriter->writeToFile($file);
        header('Content-Type: application/json');
        echo json_encode(array('filename' => base_url().'Bastb_pusat/download?filename='.$file));
    }
    ...
    public function upload_file()
    {
		...
		if($bastb_pusat->nama_file != '' and $bastb_pusat->nama_file != 'null' and !is_null($bastb_pusat->nama_file) and $bastb_pusat->nama_file != '[]')
			$nama_file = json_decode($bastb_pusat->nama_file);
		else
			$nama_file = [];
        ...
        if(count($nama_file) > 10){
		    ...
		    foreach($_FILES['file']['tmp_name'] as $key => $value) {
		        ...
		        $targetFile =  $target_file = $this->config->item('doc_root').'/upload/bastb_pusat/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
	...
	public function upload_filefoto()
    {
		...
		if($bastb_pusat->nama_filefoto != '' and $bastb_pusat->nama_filefoto != 'null' and !is_null($bastb_pusat->nama_filefoto) and $bastb_pusat->nama_filefoto != '[]')
			$nama_filefoto = json_decode($bastb_pusat->nama_filefoto);
		else
			$nama_filefoto = [];
    ...
    public function remove_file()
    {
		...
		unlink($this->config->item('doc_root').'/upload/bastb_pusat/'.$nama_file);	
	...
	public function remove_filefoto()
    {
		...
		if($bastb_pusat->nama_filefoto != '' and $bastb_pusat->nama_filefoto != 'null' and !is_null($bastb_pusat->nama_filefoto) and $bastb_pusat->nama_filefoto != '[]')
			$images = json_decode($bastb_pusat->nama_filefoto);
		else
			$images = [];

		$nama_filefoto = $images[$urutanfile];
		unlink($this->config->item('doc_root').'/upload/bastb_pusat/'.$nama_filefoto);	
