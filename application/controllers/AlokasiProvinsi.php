...
class AlokasiProvinsi extends CI_Controller {
	...
	public function index()
	{
		...
		$param['total_unit'] = $this->AlokasiProvinsiModel->GetTotalUnit();
		$param['total_nilai'] = $this->AlokasiProvinsiModel->GetTotalNilai();
		$param['total_kontrak'] = $this->AlokasiProvinsiModel->GetTotalKontrak();
		$param['total_merk'] = $this->AlokasiProvinsiModel->GetTotalMerk();

		$param['total_unit_kontrak'] = $this->KontrakProvinsiModel->GetTotalUnit();
		$param['total_nilai_kontrak'] = $this->KontrakProvinsiModel->GetTotalNilai();

		$data = array(
	        'title' => 'Data Alokasi TP Provinsi',
	        'content-path' => 'PENGADAAN TP Provinsi / ALOKASI',
	...
	public function AjaxGetAllData()
	{
		...
		$totalData = $this->AlokasiProvinsiModel->GetAllAjaxCount();
        ...
        {            
            $posts = $this->AlokasiProvinsiModel->GetAllForAjax($_POST['start'], $_POST['length'], $order, $dir, $filtercond);
            $totalFiltered = $this->AlokasiProvinsiModel->GetFilterAjaxCount($filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->AlokasiProvinsiModel->GetSearchAjax($_POST['start'], $_POST['length'], $search, $order, $dir, $filtercond);

            $totalFiltered = $this->AlokasiProvinsiModel->GetSearchAjaxCount($search);
        }
	...
	public function GetData()
	{
		...
		$data = $this->AlokasiProvinsiModel->GetData($id_kontrak_detail);
    ...
    public function AjaxGetAllDataForHibah()
    {
        ...
        {            
            ...
            $totalFiltered = $this->AlokasiProvinsiModel->GetSearchAjaxCount($search, $filtercond);
        }
    ...
    public function doExport() {
        ...
        if(empty($this->input->post('search')['value']))
        {            
            $data = $this->AlokasiProvinsiModel->ExportAllForAjax($order, $dir, $filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 
            $data =  $this->AlokasiProvinsiModel->ExportSearchAjax($search, $order, $dir, $filtercond);
        }
        ...
        $this->xlsxwriter->writeSheetHeader('Alokasi Provinsi', $visible_header_columns, array('font-style'=>'bold'));
            ...
            foreach($visible_columns as $key => $value) {
                ...
                switch($value['id']) {
                    ...
                    case 'no_adendum': 
                        ...
                        break;
                    case 'nama_penyedia': 
                        $newRow[$key] = $row['nama_penyedia_provinsi'];
                        break;
                    default: 
                        $newRow[$key] = $defaultValue;
                }
            }
            $this->xlsxwriter->writeSheetRow('Alokasi Provinsi', $newRow);
        }
        
        $uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
        $file = "upload/BASTB App Data Alokasi Provinsi - $uniq_id.xlsx";
        $this->xlsxwriter->writeToFile($file);

        header('Content-Type: application/json');
        echo json_encode(array('filename' => base_url().'AlokasiProvinsi/doDownload?filename='.$file));
    }
