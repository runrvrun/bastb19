...
class Alokasi_pusat extends CI_Controller {
    public $colsc = array(
		...
		array("column" => "termin", "caption" => "SP2D", "dbcolumn" => "termin"),
    );
    ...
    public function index($id = null)
    {
        ...
        if(!empty($id_kontrak_pusat)){
            $param['kontrak_pusat'] = $this->Kontrak_pusat_model->get($_GET['id_kontrak_pusat']);
        }
        ...
        $param['total_kontrak'] = $this->Alokasi_pusat_model->total_kontrak($id_kontrak_pusat);
        $param['total_merk'] = $this->Alokasi_pusat_model->total_merk($id_kontrak_pusat);
        ...
        $data = array(
            'title' => 'Data Alokasi Kontrak Pusat',
    ...
    { 
        ...
        // var_dump($order);exit(); 
		$posts_all_search =  $this->Alokasi_pusat_model->get(null, $id_kontrak_pusat, null, null, null, null, $filtercond);
        $totalFiltered = count($posts_all_search);
		$posts =  $this->Alokasi_pusat_model->get(null, $id_kontrak_pusat, $start, $length, $order, $dir, $filtercond);
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                if($post->regcad == 'REGULER') $tools .= '<a class="dropdown-item" href="'.base_url('Alokasi_persediaan_pusat/index?id_alokasi='.$post->id).'">Input Persediaan</a>';
                ...
                if($bolehedit)
                    $tools .= '<a class="dropdown-item" href="'.base_url('Alokasi_pusat/edit?id='.$post->id).'">Ubah Data</a>';
                if($bolehhapus)
                    $tools .= '<a class="dropdown-item"  href="#" data-toggle="modal" data-record-title="'.$post->id.'" data-target="#confirm-delete" data-href="'.base_url('Alokasi_pusat/destroy/').$post->id.'">Hapus Data</a>';
                $tools .= '</div>
                </div>';
    ...
    public function index_confirmed($id = null) 
    {
        ...
        //update id_sp2d on dropdown select
        $termin = $this->input->post('termin');
        if(!empty($termin)){
            $source = $this->input->post('source');
            $id_alokasi = $this->input->post('id_alokasi');
            if($source == 'alokasi_persediaan'){
                $this->Alokasi_persediaan_pusat_model->update($id_alokasi, array('termin'=>$termin));
                $this->session->set_flashdata('info','SP2D updated successfully.');
            }else{
                $this->Alokasi_pusat_model->update($id_alokasi, array('termin'=>$termin));
                $this->session->set_flashdata('info','SP2D updated successfully.');
            }
        }
        
        $this->load->library('parser');
        $param['alokasi_pusat'] = $this->Alokasi_pusat_model->get_confirmed();

        $param['total_unit'] = $this->Alokasi_pusat_model->total_unit_confirmed();
        $param['total_nilai'] = $this->Alokasi_pusat_model->total_nilai_confirmed();
        $param['total_unit_kontrak'] = $this->Kontrak_pusat_model->total_unit();
        $param['total_nilai_kontrak'] = $this->Kontrak_pusat_model->total_nilai();
        ...
        $data = array(
            ...
            'content-path' => 'PENGADAAN PUSAT / ALOKASI',
            'content' => $this->load->view('alokasi-pusat/index_confirmed', $param, TRUE),
        );
        $this->parser->parse('default_template', $data);
    }
    ...
    { 
        ...
        $totalData = $this->Alokasi_pusat_model->get_confirmed(null,null);
        $totalData = count($totalData);
        ...
        if(!empty($search_norangka)){
            $filtercond .= " replace_search_norangka 
            tb_norangka.norangka LIKE '".$search_norangka."' OR tb_norangka.nomesin LIKE '".$search_norangka."')";
        }
        
        $posts_all_search =  $this->Alokasi_pusat_model->get_confirmed(null, $id_kontrak_pusat, null, null, null, null, $filtercond);
        $totalFiltered = count($posts_all_search);
		$posts =  $this->Alokasi_pusat_model->get_confirmed(null, $id_kontrak_pusat, $start, $length, $order, $dir, $filtercond);
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                $nestedData['termin'] = '<form method="POST" action="'.base_url('Alokasi_pusat/index_confirmed').'">
                <input type="hidden" name="id_alokasi" value="'.$post->id.'"></input>
                <input type="hidden" name="source" value="'.$post->source.'"></input>
                <select name="termin" class="form-control" onchange="this.form.submit()">';
                    $nestedData['termin'] .= '<option value=""> -- Pilih Termin -- </option>';
                    $nestedData['termin'] .= '<option value="UANG MUKA" ';
                    $nestedData['termin'] .= ($post->termin == 'UANG MUKA')? 'selected ':'';
                    $nestedData['termin'] .= '>UANG MUKA</option>';
                for($i=1;$i<=$post->jumlah_termin;$i++){
                    $nestedData['termin'] .= '<option value="TERMIN '.$i.'" ';
                    $nestedData['termin'] .= ($post->termin == 'TERMIN '.$i)? 'selected ':'';
                    $nestedData['termin'] .= '>TERMIN '.$i.'</option>';
                }
                $nestedData['termin'] .= '</select><form>';
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">';
                if($post->source == 'alokasi'){
                    if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PROVINSI','ADMIN KABUPATEN'))){// hide menu from admin provinsi and kabupaten
                        $tools .= '<a class="dropdown-item" href="'.base_url('Baphp/index?id_alokasi='.$post->id).'">BAPHP</a>';
                    }
                    if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PENYEDIA PUSAT','ADMIN PENYEDIA PROVINSI'))){// hide menu from admin penyedia
                        $tools .= '<a class="dropdown-item" href="'.base_url('Bastb_pusat/index?id_alokasi='.$post->id).'">BASTB</a>';
                    }
                }else{
                    if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PROVINSI','ADMIN KABUPATEN'))){// hide menu from admin provinsi and kabupaten
                        $tools .= '<a class="dropdown-item" href="'.base_url('Baphp_persediaan/index?id_alokasi_persediaan='.$post->id).'">BAPHP</a>';
                    }
                    if(!in_array($this->session->userdata('logged_in')->role_pengguna,array('ADMIN PENYEDIA PUSAT','ADMIN PENYEDIA PROVINSI'))){// hide menu from admin penyedia
                        $tools .= '<a class="dropdown-item" href="'.base_url('Bastb_persediaan/index?id_alokasi_persediaan='.$post->id).'">BASTB</a>';
                    }
                }
                $tools .= '</div>
                </div>';
    ...
    public function show($id)
    {
		...
		$param['alokasi'] = array();
		$param['total_unit'] = $this->Kontrak_pusat_model->GetTotalUnitAlokasi($kontrak_pusat->id);
		$param['total_nilai'] = $this->Kontrak_pusat_model->GetTotalNilaiAlokasi($kontrak_pusat->id);

		$this->load->library('parser');
		$data = array(
	        'title' => 'Data Alokasi Kontrak Pusat',
	        'content-path' => 'PENGADAAN PUSAT / DATA ALOKASI KONTRAK',
	        'content' => $this->load->view('alokasi-pusat/index', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
    ...
    public function get_json()
    {
		...
		$data = $this->Alokasi_pusat_model->get($id_kontrak_detail);
    ...
    public function create()
	{
		...
		$data = array(
	        'title' => 'Input Alokasi Kontrak Pusat',
	        'content-path' => 'PENGADAAN PUSAT / DATA ALOKASI / TAMBAH DATA',
	        'content' => $this->load->view('alokasi-pusat/add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
        ...
        $total_unit = $this->Alokasi_pusat_model->total_unit(null,$id_kontrak_pusat);
        $total_nilai = $this->Alokasi_pusat_model->total_nilai(null,$id_kontrak_pusat);
        $kontrak_pusat = $this->Kontrak_pusat_model->get($id_kontrak_pusat);
        if(($jumlah_unit+$total_unit > $kontrak_pusat->jumlah_barang+0 || $jumlah_nilai+$total_nilai > $kontrak_pusat->nilai_barang+0)){
            $this->session->set_flashdata('error','Jumlah atau nilai unit tidak dapat melebihi jumlah atau nilai unit kontrak.');
            redirect('Alokasi_pusat/create?id_kontrak_pusat='.$id_kontrak_pusat);
        }
		...
		if($id_kontrak_pusat == '' or $id_provinsi == '' or $id_kabupaten == ''){
			$this->session->set_flashdata('error','All fields must be filled.');
     		redirect('Alokasi_pusat/create?id='.$id_kontrak_pusat);
		}
		else{
			$target_file_1 = '';
			$target_file_2 = '';
			...
			if(file_exists($_FILES['nama_file_adendum_1']['tmp_name']) and is_uploaded_file($_FILES['nama_file_adendum_1']['tmp_name'])) {
		    	...
		    	$target_file_1 = $this->config->item('doc_root').'/upload/kontrak_pusat/alokasi/'.basename($_FILES["nama_file_adendum_1"]["name"]);
			...
			if(file_exists($_FILES['nama_file_adendum_2']['tmp_name']) and is_uploaded_file($_FILES['nama_file_adendum_2']['tmp_name'])) {
		    	...
		    	$target_file_2 = $this->config->item('doc_root').'/upload/kontrak_pusat/alokasi/'.basename($_FILES["nama_file_adendum_2"]["name"]);
			...
			if(file_exists($_FILES['nama_file_adendum_3']['tmp_name']) and is_uploaded_file($_FILES['nama_file_adendum_3']['tmp_name'])) {
		    	...
		    	$target_file_3 = $this->config->item('doc_root').'/upload/kontrak_pusat/alokasi/'.basename($_FILES["nama_file_adendum_3"]["name"]);
			...
			$this->Alokasi_pusat_model->store($data);
			$this->session->set_flashdata('info','Data inserted successfully.');
			redirect('Alokasi_pusat/index?id_kontrak_pusat='.$id_kontrak_pusat);
		}
	...
	public function edit()
	{
		...
		$param['alokasi_pusat'] = $alokasi;
		$param['kontrak_pusat'] = $this->Kontrak_pusat_model->Get($alokasi->id_kontrak_pusat);
		...
		$data = array(
	        'title' => 'Data Alokasi Kontrak Pusat',
	        'content-path' => 'PENGADAAN PUSAT / DATA ALOKASI / UBAH DATA',
	        'content' => $this->load->view('alokasi-pusat/edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
        ...
        $total_unit = $this->Alokasi_pusat_model->total_unit(null,$alokasi->id_kontrak_pusat);
        $total_nilai = $this->Alokasi_pusat_model->total_nilai(null,$alokasi->id_kontrak_pusat);
        $kontrak_pusat = $this->Kontrak_pusat_model->get($alokasi->id_kontrak_pusat);
        // var_dump($total_nilai);var_dump($jumlah_nilai);var_dump($alokasi->nilai_barang_rev);var_dump($kontrak_pusat->nilai_barang);exit();
        if(($total_unit-$alokasi->jumlah_barang_rev+$jumlah_unit > $kontrak_pusat->jumlah_barang+0) || $total_nilai-$alokasi->nilai_barang_rev+$jumlah_nilai > $kontrak_pusat->nilai_barang+0){
            $this->session->set_flashdata('error','Jumlah atau nilai unit tidak dapat melebihi jumlah atau nilai unit kontrak.');
            redirect('Alokasi_pusat/edit?id='.$id);
        }

		$id_kontrak_pusat = $alokasi->id_kontrak_pusat;

		$kontrak_pusat = $this->Kontrak_pusat_model->Get($id_kontrak_pusat);
		$jumlah_barang_kontrak_pusat = $kontrak_pusat->jumlah_barang;
		$jumlah_alokasi = $this->Alokasi_pusat_model->HitungJumlahAlokasiLainnya($id_kontrak_pusat, $id);
		...
		if($id_kontrak_pusat == '' or $id_provinsi == '' or $id_kabupaten == ''){
			...
			$this->Alokasi_pusat_model->update($id, $data);
	...
	{	
		...
		foreach($images as $image){
			unlink($this->config->item('doc_root').'/upload/alokasi_pusat/'.$image);	
		}

		$this->Alokasi_pusat_model->destroy($id);
    ...
    public function export_confirmed()
    {
        ...
        if(empty($this->input->post('search')['value'])){            
            $data = $this->Alokasi_pusat_model->get_confirmed(null,null,null,null,$order, $dir, $filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 
            $data =  $this->Alokasi_pusat_model->get_confirmed(null,$null,null,null, $order, $dir, $filtercond,$search);
        }
        ...
        $this->xlsxwriter->writeSheetHeader('Rekap Alokasi Pusat', $visible_header_columns, array('font-style'=>'bold'));
        ...
        foreach($data as $row) {
            ...
            $this->xlsxwriter->writeSheetRow('Rekap Alokasi Pusat', $newRow);
        }
        
        $uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
        $file = "tmp_export/Rekap Alokasi Pusat - $uniq_id.xlsx";
        $this->xlsxwriter->writeToFile($file);
        header('Content-Type: application/json');
        echo json_encode(array('filename' => base_url().'Alokasi_pusat/download?filename='.$file));
    }
    ...
    public function upload_file()
    {
		...
		$alokasi = $this->Alokasi_pusat_model->get($id_alokasi);
        ...
        if(count($nama_file) > 10){
		    ...
		    foreach($_FILES['file']['tmp_name'] as $key => $value) {
		        ...
		        $targetFile =  $target_file = $this->config->item('doc_root').'/upload/alokasi_pusat/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
			...
			$this->Alokasi_pusat_model->update($id_alokasi, $data);
    ...
    public function remove_file()
    {
		...
		unlink($this->config->item('doc_root').'/upload/alokasi_pusat/'.$nama_file);	
    ...
    public function index_hibah_json($id_kontrak_pusat = null)
    {
        $columns = array( 
            0 =>'tahun_anggaran', 
            1 =>'no_kontrak',
        ...
        $totalData = $this->Alokasi_pusat_model->get_confirmed();
        ...
        {   
            foreach ($posts as $post)
            {
                ...
                // }
                // else if($post->status_alokasi == 'DATA ADDENDUM 1'){
                ...
                // }
                // else if($post->status_alokasi == 'DATA ADDENDUM 2'){
                ...
                // }
                // else if($post->status_alokasi == 'DATA ADDENDUM 3'){
                //     $nestedData['alokasi'] = number_format($post->jumlah_barang_rev_3, 0);
                //     $nestedData['nilai_barang'] = number_format($post->nilai_barang_rev_3, 0);

                //     if($post->jumlah_barang_rev_3 == 0)
                //         $nestedData['harga_satuan'] = 0;
                //     else
                //         $nestedData['harga_satuan'] = number_format(($post->nilai_barang_rev_3/$post->jumlah_barang_rev_3), 0);
