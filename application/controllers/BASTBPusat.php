...
class BASTBPusat extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('BASTBPusatModel');
	...
	public function index()
	{
		...
		$param['bastb_pusat'] = array();
		$param['total_unit'] = $this->BASTBPusatModel->GetTotalUnit();
		$param['total_nilai'] = $this->BASTBPusatModel->GetTotalNilai();
	...
	public function AjaxGetAllData()
	{
		...
		$totalData = $this->BASTBPusatModel->GetAllAjaxCount();
        ...
        {            
            $posts = $this->BASTBPusatModel->GetAllForAjax($_POST['start'], $_POST['length'], $order, $dir, $filtercond);
            $totalFiltered = $this->BASTBPusatModel->GetFilterAjaxCount($filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->BASTBPusatModel->GetSearchAjax($_POST['start'], $_POST['length'], $search, $order, $dir, $filtercond);

            $totalFiltered = $this->BASTBPusatModel->GetSearchAjaxCount($search, $filtercond);
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                if($bolehedit)
                	$tools .= "<a class='btn btn-xs btn-primary btn-sm' href='".base_url('BASTBPusat/Edit?id=').$post->id."'><i class='glyphicon glyphicon-pencil'></i></a><a class='btn btn-xs btn-info btn-sm' data-href='#' data-toggle='modal' data-record-title='".$post->no_bastb."' data-target='#upload-modal' data-record-id='".$post->id."'><i class='glyphicon glyphicon-open-file'></i></a>";

                if($bolehhapus)
                	$tools .= "<a class='btn btn-xs btn-danger btn-sm' data-href='".base_url('BASTBPusat/doDelete?id=').$post->id."' data-toggle='modal' data-record-title='".$post->no_bastb."' data-target='#confirm-delete'><i class='glyphicon glyphicon-trash'></i></a>";
	...
	public function Add()
	{
		...
		$data = array(
	        ...
	        'content-path' => 'PEGADAAN PUSAT / BASTB / TAMBAH DATA',
	...
	{	
		...
		if($tahun_anggaran == '' or $pihak_penyerah == '' or $no_kontrak == '' or $nama_barang == '' or $merk == '' or$jumlah_barang == '' or $nilai_barang == '' or $tanggal == ''){
			...
			$this->BASTBPusatModel->Insert($data);
	...
	public function Test()
	{
			...
			$bastb_pusat = $this->BASTBPusatModel->Get($id);
	        ...
	        if(count($nama_file) > 10){
			    ...
			    foreach($_FILES['file']['tmp_name'] as $key => $value) {
			        ...
			        $targetFile =  $target_file = $_SERVER['DOCUMENT_ROOT'].'/upload/bastb_pusat/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
				...
				$this->BASTBPusatModel->Update($id, $data);
	...
	public function RemoveImage()
	{
		...
		unlink($_SERVER['DOCUMENT_ROOT'].'/upload/bastb_pusat/'.$nama_file);	
	...
	public function Edit()
	{
		...
		$param['bastb_pusat'] = $this->BASTBPusatModel->Get($id);
		...
		$data = array(
	        ...
	        'content-path' => 'PEGADAAN PUSAT / BASTB / UBAH DATA',
	...
	{	
		...
		if($tahun_anggaran == '' or $pihak_penyerah == '' or $no_kontrak == '' or $nama_barang == '' or $merk == '' or$jumlah_barang == '' or $nilai_barang == '' or $tanggal == ''){
			$this->session->set_flashdata('error','All fields must be filled.');
     		redirect('BASTBPusat/Edit?id='.$id);
		}
		else{
			$tanggal = DateTime::createFromFormat('d-m-Y', $tanggal)->format('Y-m-d');
	...
	{	
		...
		foreach($images as $image){
			unlink($_SERVER['DOCUMENT_ROOT'].'/upload/bastb_pusat/'.$image);	
		}
		
		$this->BASTBPusatModel->Delete($id);
	...
	public function GetBASTB()
	{
		...
		$data = $this->BASTBPusatModel->Get($id_bastb);
	...
	public function doExport() {
		...
		if(empty($this->input->post('search')['value']))
		{            
			$data = $this->BASTBPusatModel->ExportAllForAjax($order, $dir, $filtercond);
		}
		else {
			$search = $this->input->post('search')['value']; 
			$data =  $this->BASTBPusatModel->ExportSearchAjax($search, $order, $dir, $filtercond);
		}
		...
		$this->xlsxwriter->writeSheetHeader('BASTB Pusat', $visible_header_columns, array('font-style'=>'bold'));
		...
		foreach($data as $row) {
			...
			$this->xlsxwriter->writeSheetRow('BASTB Pusat', $newRow);
		}

		$uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
		$file = "upload/BASTB App Data BASTB Pusat - $uniq_id.xlsx";
		$this->xlsxwriter->writeToFile($file);

		header('Content-Type: application/json');
		echo json_encode(array('filename' => base_url().'BASTBPusat/doDownload?filename='.$file));
	}
