...
class BAPSTHPProvinsi extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('BAPSTHPProvinsiModel');
	...
	public function index()
	{
		...
		$param['bapsthp_provinsi'] = array();
		$param['total_unit'] = $this->BAPSTHPProvinsiModel->GetTotalUnit();
		$param['total_nilai'] = $this->BAPSTHPProvinsiModel->GetTotalNilai();
		...
		$data = array(
	        'title' => 'BAP-STHP',
	        'content-path' => 'PENGADAAN TP PROVINSI / BAP-STHP',
	        'content' => $this->load->view('bapsthp-provinsi-index', $param, TRUE),
		);
	...
	public function AjaxGetAllData()
	{
		...
		$totalData = $this->BAPSTHPProvinsiModel->GetAllAjaxCount();
        ...
        {            
            $posts = $this->BAPSTHPProvinsiModel->GetAllForAjax($_POST['start'], $_POST['length'], $order, $dir, $filtercond);
            $totalFiltered = $this->BAPSTHPProvinsiModel->GetFilterAjaxCount($filtercond);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->BAPSTHPProvinsiModel->GetSearchAjax($_POST['start'], $_POST['length'], $search, $order, $dir, $filtercond);

            $totalFiltered = $this->BAPSTHPProvinsiModel->GetSearchAjaxCount($search, $filtercond);
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
				...
				if($bolehedit)
					$tools .= "<a class='btn btn-xs btn-primary btn-sm' href='".base_url('BAPSTHPProvinsi/Edit?id=').$post->id."'><i class='glyphicon glyphicon-pencil'></i></a><a class='btn btn-xs btn-info btn-sm' data-href='#' data-toggle='modal' data-record-title='".$post->no_bapsthp."' data-target='#upload-modal' data-record-id='".$post->id."'><i class='glyphicon glyphicon-open-file'></i></a>";

				if($bolehhapus)
					$tools .= "<a class='btn btn-xs btn-danger btn-sm' data-href='".base_url('BAPSTHPProvinsi/doDelete?id=').$post->id."' data-toggle='modal' data-record-title='".$post->no_bapsthp."' data-target='#confirm-delete'><i class='glyphicon glyphicon-trash'></i></a>";
	...
	public function Add()
	{
		...
		$data = array(
	        ...
	        'content-path' => 'PEGADAAN TP PROVINSI / BAP-STHP / TAMBAH DATA',
	        'content' => $this->load->view('bapsthp-provinsi-add', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	}
	...
	{	
		...
		if($tahun_anggaran == '' or $id_penyerah == '' or $no_kontrak == '' or $nama_barang == '' or $merk == '' or$jumlah_barang == '' or $nilai_barang == '' or $tanggal == ''){
			...
			$this->BAPSTHPProvinsiModel->Insert($data);
			$this->session->set_flashdata('info','Data inserted successfully.');
			redirect('BAPSTHPProvinsi');
	...
	public function Test()
	{
			...
			$bapsthp_provinsi = $this->BAPSTHPProvinsiModel->Get($id);

			$kodefile_upload = strtotime(NOW);

			if($bapsthp_provinsi->nama_file != '' and $bapsthp_provinsi->nama_file != 'null' and !is_null($bapsthp_provinsi->nama_file) and $bapsthp_provinsi->nama_file != '[]')
				$nama_file = json_decode($bapsthp_provinsi->nama_file);
			else
				$nama_file = [];
	        ...
	        if(count($nama_file) > 10){
			    ...
			    foreach($_FILES['file']['tmp_name'] as $key => $value) {
			        ...
			        $targetFile =  $target_file = $_SERVER['DOCUMENT_ROOT'].'/upload/bapsthp_provinsi/'.$kodefile_upload.basename($_FILES['file']['name'][$key]);
				...
				$this->BAPSTHPProvinsiModel->Update($id, $data);
	...
	public function RemoveImage()
	{
		...
		if($bapsthp_provinsi->nama_file != '' and $bapsthp_provinsi->nama_file != 'null' and !is_null($bapsthp_provinsi->nama_file) and $bapsthp_provinsi->nama_file != '[]')
			$images = json_decode($bapsthp_provinsi->nama_file);
		else
			$images = [];

		$nama_file = $images[$urutanfile];
		unlink($_SERVER['DOCUMENT_ROOT'].'/upload/bapsthp_provinsi/'.$nama_file);	
	...
	public function Edit()
	{
		...
		$param['bapsthp_provinsi'] = $this->BAPSTHPProvinsiModel->Get($id);
		...
		$data = array(
	        ...
	        'content-path' => 'PEGADAAN TP PROVINSI / BAP-STHP / UBAH DATA',
	        'content' => $this->load->view('bapsthp-provinsi-edit', $param, TRUE),
		);
		$this->parser->parse('default_template', $data);
	...
	{	
		...
		foreach($images as $image){
			unlink($_SERVER['DOCUMENT_ROOT'].'/upload/bapsthp_provinsi/'.$image);	
		}
		
		$this->BAPSTHPProvinsiModel->Delete($id);
	...
	public function GetBAPSTHP()
	{
		...
		$data = $this->BAPSTHPProvinsiModel->Get($id_bapsthp);
	...
	public function doExport() {
		...
		$data = array();
		if(empty($this->input->post('search')['value'])){            
			$data = $this->BAPSTHPProvinsiModel->ExportAllForAjax($order, $dir, $filtercond);
		}
		else {
			$search = $this->input->post('search')['value'];
			$data =  $this->BAPSTHPProvinsiModel->ExportSearchAjax($search, $order, $dir, $filtercond);
		}
		...
		$this->xlsxwriter->writeSheetHeader('BAPSTHP Provinsi', $visible_header_columns, array('font-style'=>'bold'));
		...
		foreach($data as $row) {
			...
			$this->xlsxwriter->writeSheetRow('BAPSTHP Provinsi', $newRow);
		}

		$uniq_id = substr(md5(uniqid(rand(), true)), 0, 5);
		$file = "upload/BASTB App Data BAPSTHP Provinsi - $uniq_id.xlsx";
		$this->xlsxwriter->writeToFile($file);

		header('Content-Type: application/json');
		echo json_encode(array('filename' => base_url().'BAPSTHPProvinsi/doDownload?filename='.$file));
	}
